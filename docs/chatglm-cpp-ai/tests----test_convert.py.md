...
# 从 chatglm_cpp.convert 模块中导入 quantize_q4_0, quantize_q4_1, quantize_q5_0, quantize_q5_1, quantize_q8_0 函数

# 获取当前文件所在目录的绝对路径
HERE = Path(__file__).resolve().parent

# 以下是通过 torch.manual_seed(0) 和 torch.randn(2, 128) 生成的权重数据
...
# 定义一个测试函数，对 q8_0 类型的权重进行量化
def test_quantize_q8_0():
    # 对权重进行 q8_0 类型的量化，并转换为整数类型
    q_tensor = quantize_q8_0(weight).int()
    # 格式化开关关闭，下面的代码块不受格式化影响
    # fmt: off
    # 创建一个与 q_tensor 形状相同的张量，包含预先定义的数值
...
# 测试量化函数 quantize_q4_0，将权重量化为 Q4.0 格式的张量
def test_quantize_q4_0():
    # 使用 quantize_q4_0 函数将权重量化为 Q4.0 格式的张量
    q_tensor = quantize_q4_0(weight).int()
    # fmt: off
    # 定义期望的量化结果张量 ggml_q_tensor
    ggml_q_tensor = torch.tensor([
    ...
    # fmt: on
    # 断言量化结果张量与期望的量化结果张量相等
    assert (q_tensor == ggml_q_tensor).all()


# 测试量化函数 quantize_q4_1
def test_quantize_q4_1():
    # 使用 quantize_q4_1 函数将权重量化为 Q4.1 格式的张量
    q_tensor = quantize_q4_1(weight).int()
    # fmt: off
    # 定义一个张量 ggml_q_tensor，包含了一系列数值
    ggml_q_tensor = torch.tensor([
    ...
    # 断言两个张量的所有元素是否相等
    assert (q_tensor == ggml_q_tensor).all()
# 测试量化函数 quantize_q5_0，将权重量化为 Q5.0 格式的张量
def test_quantize_q5_0():
    # 使用 quantize_q5_0 函数将权重量化为 Q5.0 格式的张量
    q_tensor = quantize_q5_0(weight).int()
    # 格式化代码关闭，用于忽略代码格式化工具对接下来的代码的影响
    # fmt: off
    # 创建期望的 Q5.0 格式的张量 ggml_q_tensor
    ggml_q_tensor = torch.tensor([
...
# 测试量化函数 quantize_q5_1
def test_quantize_q5_1():
    # 使用 quantize_q5_1 函数将权重量化为 Q5.1 格式的张量
    q_tensor = quantize_q5_1(weight).int()
    # 格式化代码关闭
    # fmt: off
    # 定义一个张量 ggml_q_tensor，包含给定的数值，并且形状与 q_tensor 相同
    ggml_q_tensor = torch.tensor([
    ...
    # 使用断言检查 q_tensor 和 ggml_q_tensor 是否完全相等
    assert (q_tensor == ggml_q_tensor).all()
# 定义 CHATGLM_MODEL_PATH 变量，指向 chatglm 模型的路径
CHATGLM_MODEL_PATH = Path(
    "~/.cache/huggingface/hub/models--THUDM--chatglm-6b/snapshots/619e736c6d4cd139840579c5482063b75bed5666"
).expanduser()

# 定义 CHATGLM2_MODEL_PATH 变量，指向 chatglm2 模型的路径
...
# 定义 BAICHUAN7B_MODEL_PATH 变量，指向 Baichuan2-7B-Chat 模型的路径
...
# 定义 BAICHUAN13B_MODEL_PATH 变量，指向 Baichuan-13B-Chat 模型的路径
...
# 定义 INTERNLM_MODEL_PATH 变量，指向 internlm-chat-7b-v1_1 模型的路径
...
# 定义 make_data_embedding 函数，用于创建数据的嵌入
def make_data_embedding():
    # 创建一个 Embedding 层，输入维度为 4，输出维度为 3
    m = torch.nn.Embedding(4, 3)
    # 创建一个输入张量 x
    x = torch.tensor([1, 3, 0, 2, 3])
    # 将输入张量 x 通过 Embedding 层 m 得到输出张量 y
    y = m(x)
    # 打印权重矩阵 m 的数据展平后的结果
...
# 定义 make_data_linear 函数，用于创建线性数据
def make_data_linear():
    # 创建一个形状为 (16, 32) 的随机权重张量 w
    w = torch.randn(16, 32)
    # 创建一个形状为 (16,) 的随机偏置张量 b
    b = torch.randn(16)
    # 创建一个形状为 (2, 32) 的随机输入张量 x
    x = torch.randn(2, 32)
    # 使用线性函数 F.linear 计算输入张量 x 经过权重 w 和偏置 b 的线性变换后的输出张量 y
...
# 定义 make_data_layernorm 函数，用于创建 LayerNorm 数据
def make_data_layernorm():
    # 创建一个形状为 (64,) 的随机权重张量 w
    w = torch.randn(64)
    # 创建一个形状为 (64,) 的随机偏置张量 b
    b = torch.randn(64)
    # 创建一个形状为 (3, 64) 的随机输入张量 x
    x = torch.randn(3, 64)
    # 使用 LayerNorm 函数 F.layer_norm 对输入张量 x 进行 LayerNorm 处理，得到输出张量 y
...
# 定义 make_data_rms_norm 函数，用于创建 RMSNorm 数据
def make_data_rms_norm():
    # 将 CHATGLM2_MODEL_PATH 添加到系统路径中
    sys.path.append(str(CHATGLM2_MODEL_PATH))
    # 从 modeling_chatglm 模块中导入 RMSNorm 类
    from modeling_chatglm import RMSNorm

    # 创建一个 RMSNorm 实例 m，输入维度为 64，eps 为 1e-5，并设置为评估模式
    ...
    # 创建一个形状为 (3, 64) 的随机输入张量 x
    x = torch.randn(3, 64)
    # 使用 RMSNorm 实例 m 对输入张量 x 进行处理，得到输出张量 y
    with torch.no_grad():
        y = m(x)
    # 使用上下文管理器打开文件 "data/rms_norm.data" 以二进制写入模式
    with open(HERE / "data/rms_norm.data", "wb") as f:
        # 将 m 的权重数据以 numpy 数组的形式写入文件
        m.weight.data.numpy().tofile(f)
        # 将 x 的数据以 numpy 数组的形式写入文件
        x.numpy().tofile(f)
        # 将 y 的数据以 numpy 数组的形式写入文件
        y.numpy().tofile(f)
def make_data_glm_model():
    # 将CHATGLM_MODEL_PATH转换为字符串后添加到系统路径中
    sys.path.append(str(CHATGLM_MODEL_PATH))
    # 从模块modeling_chatglm中导入ChatGLMModel类
    from modeling_chatglm import ChatGLMModel
    # 从transformers模块中导入AutoConfig类

    # 从CHATGLM_MODEL_PATH加载预训练配置，信任远程代码
    ...
    # 创建ChatGLMModel模型实例，不使用空初始化，转换为浮点型，设置为评估模式
...
def make_data_glm2_model():
    ...
    # 设置模型的torch数据类型为float32
    config.torch_dtype = torch.float32

    # 创建ChatGLMModel模型实例，并转换为浮点型，设置为评估模式
    m = ChatGLMModel(config).float().eval()
    # 初始化模型参数为在[-0.5, 0.5]之间均匀分布的值
    for param in m.parameters():
        param.data.uniform_(-0.5, 0.5)
    ...
    with open(HERE / "data/glm2_model.data", "wb") as f:
        ...
        # 将编码器的最终归一化层权重数据转换为 numpy 数组，并写入文件 f
        m.encoder.final_layernorm.weight.data.numpy().tofile(f)
        # 将 x1 转换为整数，然后转换为 numpy 数组，并写入文件 f
        x1.int().numpy().tofile(f)
        # 将 y1 转换为 numpy 数组，并写入文件 f
        y1.numpy().tofile(f)
        # 将 x2 转换为整数，然后转换为 numpy 数组，并写入文件 f
        x2.int().numpy().tofile(f)
        # 将 y2 转换为 numpy 数组，并写入文件 f
        y2.numpy().tofile(f)
        # 将 x3 转换为整数，然后转换为 numpy 数组，并写入文件 f
        x3.int().numpy().tofile(f)
        # 将 y3 转换为 numpy 数组，并写入文件 f
        y3.numpy().tofile(f)
# 定义一个函数，用于创建一个特定的 ChatGLM3 模型
def make_data_glm3_model():
    ...
    # self attention
    # 创建一个长度为 seq_len 的整数张量 x1
    x1 = torch.arange(seq_len, dtype=torch.int64)[None, :]
    # 创建一个长度为 seq_len 的位置编码张量
    position_ids = torch.arange(seq_len, dtype=torch.int64)[None, :]
    # 创建一个形状为 (1, seq_len) 的全1张量作为注意力掩码
    attn_mask = torch.ones(1, seq_len, dtype=torch.int64)
    # 使用模型进行推理，得到输出和过去的键值对
    with torch.no_grad():
        out = m(x1, position_ids=position_ids, attention_mask=attn_mask, use_cache=True)
        y1 = out.last_hidden_state
        kv_cache = out.past_key_values
    ...
    # cross attention
    # 创建一个形状为 (1, 1) 的整数张量 x2
    x2 = torch.tensor([[seq_len]], dtype=torch.int64)
    # 创建一个形状为 (1, 1) 的位置编码张量
    position_ids = torch.tensor([[seq_len]], dtype=torch.int64)
    # 创建一个形状为 (1, seq_len+1) 的全1张量作为注意力掩码
    attn_mask = torch.ones(1, seq_len + 1, dtype=torch.int64)
    # 使用模型进行推理，得到输出和过去的键值对
    with torch.no_grad():
        out = m(x2, position_ids=position_ids, attention_mask=attn_mask, past_key_values=kv_cache, use_cache=True)
    ...
    # cross attention
    # 创建一个形状为 (1, 1) 的整数张量 x3
    x3 = torch.tensor([[seq_len + 1]], dtype=torch.int64)
    # 创建一个形状为 (1, 1) 的位置编码张量
    position_ids = torch.tensor([[seq_len + 1]], dtype=torch.int64)
    # 创建一个形状为 (1, seq_len+2) 的全1张量作为注意力掩码
    attn_mask = torch.ones(1, seq_len + 2, dtype=torch.int64)
    # 使用模型进行推理，得到输出和过去的键值对
    with torch.no_grad():
        out = m(x3, position_ids=position_ids, attention_mask=attn_mask, past_key_values=kv_cache, use_cache=True)
    ...
    # 以二进制写入模式打开文件 "data/glm3_model.data"
    with open(HERE / "data/glm3_model.data", "wb") as f:
        # 将 m 中的 word_embeddings 权重数据以 numpy 数组形式写入文件
        m.embedding.word_embeddings.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].input_layernorm 权重数据以 numpy 数组形式写入文件
        m.encoder.layers[0].input_layernorm.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].self_attention.query_key_value 权重数据以 numpy 数组形式写入文件
        m.encoder.layers[0].self_attention.query_key_value.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].self_attention.query_key_value 偏置数据以 numpy 数组形式写入文件
        m.encoder.layers[0].self_attention.query_key_value.bias.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].self_attention.dense 权重数据以 numpy 数组形式写入文件
        m.encoder.layers[0].self_attention.dense.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].post_attention_layernorm 权重数据以 numpy 数组形式写入文件
        m.encoder.layers[0].post_attention_layernorm.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].mlp.dense_h_to_4h 权重数据以 numpy 数组形式写入文件
        m.encoder.layers[0].mlp.dense_h_to_4h.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.layers[0].mlp.dense_4h_to_h 权重数据以 numpy 数组形式写入文件
        m.encoder.layers[0].mlp.dense_4h_to_h.weight.data.numpy().tofile(f)
        # 将 m 中的 encoder.final_layernorm 权重数据以 numpy 数组形式写入文件
        m.encoder.final_layernorm.weight.data.numpy().tofile(f)

        # 将 x1 转换为整数后以 numpy 数组形式写入文件
        x1.int().numpy().tofile(f)
        # 将 y1 以 numpy 数组形式写入文件
        y1.numpy().tofile(f)
        # 将 x2 转换为整数后以 numpy 数组形式写入文件
        x2.int().numpy().tofile(f)
        # 将 y2 以 numpy 数组形式写入文件
        y2.numpy().tofile(f)
        # 将 x3 转换为整数后以 numpy 数组形式写入文件
        x3.int().numpy().tofile(f)
        # 将 y3 以 numpy 数组形式写入文件
        y3.numpy().tofile(f)
# 定义一个函数，用于生成白船模型数据
def _make_data_baichuan_model(model_path, out_name):
    # 将模型路径添加到系统路径中
    sys.path.append(str(model_path))
    # 从模型文件中导入白船模型和AutoConfig
    from modeling_baichuan import BaichuanModel
    from transformers import AutoConfig

    # 从预训练模型路径加载配置信息，信任远程代码
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    # 设置配置信息中的隐藏层大小为32
    config.hidden_size = 32
    # 设置配置信息中的注意力头数为8
    config.num_attention_heads = 8
    # 设置配置信息中的中间层大小为隐藏层大小的3倍
    config.intermediate_size = config.hidden_size * 3
    # 设置配置信息中的隐藏层层数为1
    config.num_hidden_layers = 1
    # 设置配置信息中的torch数据类型为float32
    config.torch_dtype = torch.float32
    # 设置配置信息中的词汇表大小为5
    config.vocab_size = 5
    ...
    with open(HERE / f"data/{out_name}", "wb") as f:
        # 将模型的嵌入层权重数据以 numpy 数组的形式写入文件
        m.embed_tokens.weight.data.numpy().tofile(f)
        # 将模型第一层的输入层归一化权重数据以 numpy 数组的形式写入文件
        m.layers[0].input_layernorm.weight.data.numpy().tofile(f)
        # 将模型第一层的自注意力机制的 W_pack 权重数据以 numpy 数组的形式写入文件
        m.layers[0].self_attn.W_pack.weight.data.numpy().tofile(f)
        # 将模型第一层的自注意力机制的 o_proj 权重数据以 numpy 数组的形式写入文件
        m.layers[0].self_attn.o_proj.weight.data.numpy().tofile(f)
        # 将模型第一层的注意力后的归一化权重数据以 numpy 数组的形式写入文件
        m.layers[0].post_attention_layernorm.weight.data.numpy().tofile(f)
        # 将模型第一层的多层感知机的 gate_proj 权重数据以 numpy 数组的形式写入文件
        m.layers[0].mlp.gate_proj.weight.data.numpy().tofile(f)
        # 将模型第一层的多层感知机的 down_proj 权重数据以 numpy 数组的形式写入文件
        m.layers[0].mlp.down_proj.weight.data.numpy().tofile(f)
        # 将模型第一层的多层感知机的 up_proj 权重数据以 numpy 数组的形式写入文件
        m.layers[0].mlp.up_proj.weight.data.numpy().tofile(f)
        # 将模型的归一化权重数据以 numpy 数组的形式写入文件
        m.norm.weight.data.numpy().tofile(f)
    
        # 将 x1 转换为整数后以 numpy 数组的形式写入文件
        x1.int().numpy().tofile(f)
        # 将 y1 以 numpy 数组的形式写入文件
        y1.numpy().tofile(f)
        # 将 x2 转换为整数后以 numpy 数组的形式写入文件
        x2.int().numpy().tofile(f)
        # 将 y2 以 numpy 数组的形式写入文件
        y2.numpy().tofile(f)
        # 将 x3 转换为整数后以 numpy 数组的形式写入文件
        x3.int().numpy().tofile(f)
        # 将 y3 以 numpy 数组的形式写入文件
        y3.numpy().tofile(f)
# 创建白船7B模型数据
...
def make_internlm_model():
    # 将INTERNLM_MODEL_PATH添加到系统路径中
    sys.path.append(str(INTERNLM_MODEL_PATH))
    # 导入InternLMModel和AutoConfig
    ...
    # 创建InternLMModel实例并转换为浮点型，设置为评估模式
    ...
    with torch.no_grad():
        # 使用模型 m 进行推理，传入输入 x3，注意力掩码 attn_mask，位置编码 position_ids，缓存 kv_cache，并启用缓存
    ...
    # 打开文件 "data/internlm_model.data" 以二进制写入模式
    with open(HERE / f"data/internlm_model.data", "wb") as f:
        # 将模型 m 的嵌入层权重数据写入文件
        m.embed_tokens.weight.data.numpy().tofile(f)
        # 将模型 m 第一层的输入层归一化权重数据写入文件
        m.layers[0].input_layernorm.weight.data.numpy().tofile(f)
        # 将模型 m 第一层的 QKV 投影权重数据写入文件
        qkv_proj = torch.cat(
            (
        ...
        # 将模型 m 第一层的输出投影权重数据写入文件
        m.layers[0].self_attn.o_proj.weight.data.numpy().tofile(f)
        # 将模型 m 第一层的输出投影偏置数据写入文件
        m.layers[0].self_attn.o_proj.bias.data.numpy().tofile(f)
        # 将模型 m 第一层的注意力后归一化权重数据写入文件
        m.layers[0].post_attention_layernorm.weight.data.numpy().tofile(f)
        # 将模型 m 第一层的 MLP 门控投影权重数据写入文件
        m.layers[0].mlp.gate_proj.weight.data.numpy().tofile(f)
        # 将模型 m 第一层的 MLP 上层投影权重数据写入文件
        m.layers[0].mlp.up_proj.weight.data.numpy().tofile(f)
        # 将模型 m 第一层的 MLP 下层投影权重数据写入文件
        m.layers[0].mlp.down_proj.weight.data.numpy().tofile(f)
        # 将模型 m 归一化层权重数据写入文件
        m.norm.weight.data.numpy().tofile(f)
...
def main():
    # 设置随机种子为0
    torch.manual_seed(0)
    # 创建一个目录，如果父目录不存在则创建父目录，如果目录已存在则不报错
    (HERE / "data").mkdir(parents=True, exist_ok=True)
    # 调用 make_data_linear() 函数
    # 调用 make_data_layernorm() 函数
    # 调用 make_data_rms_norm() 函数
    # 调用 make_data_glm_model() 函数
    # 调用 make_data_glm2_model() 函数
    # 调用 make_data_glm3_model() 函数
    make_data_glm3_model()
    # 调用 make_data_baichuan7b_model() 函数
    # 调用 make_data_baichuan13b_model() 函数
    # 调用 make_internlm_model() 函数
