...
#if defined(_MSC_VER)
#pragma warning(disable: 4244 4267) // 禁止警告 4244 和 4267：可能丢失数据
#endif

#define MAX_ALIGNMENT 64  // 定义最大对齐值为 64
#define QK 32  // 定义 QK 值为 32
...
struct quantize_perf_params {
    ...
    std::vector<size_t> test_sizes;  // 测试大小的大小类型向量
    size_t alignment_offset = 0;  // 对齐偏移量默认为 0
    bool op_quantize_row_q_reference = false;  // 是否进行参考量化行 q 操作，默认为 false
    bool op_quantize_row_q = false;  // 是否进行量化行 q 操作，默认为 false
    bool op_dequantize_row_q = false;  // 是否进行反量化行 q 操作，默认为 false
    bool op_quantize_row_q_dot = false;  // 是否进行量化行 q 点乘操作，默认为 false
    bool op_vec_dot_q = false;  // 是否进行向量点乘 q 操作，默认为 false
...
#ifdef __POPCNT__
    unsigned int dummy;
    return __rdtscp(&dummy);  // 返回时间戳计数
#else
    return __rdtsc();  // 返回时间戳计数
#endif
}

#else

#define cpu_cycles() 0  // 定义 cpu_cycles 函数为 0
...
static float gigabytes_per_second(size_t bytes, int64_t usecs) {
    return bytes / (float) usecs * 1000000 / (1024*1024*1024);  // 计算每秒的吉比字节数
}

// 对齐指针并添加偏移量
static void * align_with_offset(void * ptr, int offset) {
    size_t dummy_size = MAX_ALIGNMENT * 4;
    return (char *) std::align(MAX_ALIGNMENT, MAX_ALIGNMENT, ptr, dummy_size) + offset;  // 对齐指针并添加偏移量
}
...
int main(int argc, char * argv[]) {
    ...
    // 如果没有设置任何量化或者点积操作，则将所有操作都设置为 true
