...
#if defined(_MSC_VER)
#pragma warning(disable: 4244 4267) // 可能的数据丢失
#endif
...
constexpr float MAX_QUANTIZATION_TOTAL_ERROR_2BITS = 0.0075f;  // 定义最大量化总误差（2位）
constexpr float MAX_QUANTIZATION_TOTAL_ERROR_3BITS = 0.0040f;  // 定义最大量化总误差（3位）
...
static float total_quantization_error(ggml_type_traits_t & qfns, size_t test_size, const float * test_data) {
    ...
    qfns.from_float(test_data, tmp_q.data(), test_size);  // 将浮点数转换为量化数
    qfns.to_float(tmp_q.data(), tmp_out.data(), test_size);  // 将量化数转换为浮点数
...
static float reference_quantization_error(ggml_type_traits_t & qfns, size_t test_size, const float * test_data) {
    ...
    qfns.from_float_reference(test_data, tmp_q.data(), test_size);  // 使用参考方法将浮点数转换为量化数

... (以下部分代码未提供，继续按照相同的格式添加注释)
    # 将 tmp_q.data() 转换为浮点数，并存储到 tmp_out_ref.data() 中，使用 test_size 指定的大小
    qfns.to_float(tmp_q.data(), tmp_out_ref.data(), test_size);
    # 返回 tmp_out.data() 和 tmp_out_ref.data() 之间的均方根误差，使用 test_size 指定的大小
