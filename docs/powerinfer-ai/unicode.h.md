...
# 定义一系列范围，每个范围包含两个十六进制数，表示 Unicode 字符的范围
...
# 定义一系列 Unicode 字符的范围，用大括号包围，每个范围由起始值和结束值组成
...
# 定义一系列范围，每个范围包含一段 Unicode 字符的编码范围
...
# 定义一系列十六进制范围，用于表示 Unicode 字符的编码范围
...
# 定义一系列范围，每个范围由两个16进制数表示，用来描述 Unicode 字符的范围
...
// 定义 Unicode 编码范围，每个元素为一对十六进制数，表示一个范围的起始和结束编码
...
// 定义标点符号的 Unicode 编码范围，每个元素为一对十六进制数，表示一个范围的起始和结束编码
...
# 定义一系列范围，每个范围由两个十六进制数表示，用来表示 Unicode 字符范围
...
static std::string codepoint_to_utf8(uint32_t cp) {
    // 初始化结果字符串
    std::string result;
    // 如果 Unicode 码点在 0x00 到 0x7f 之间
    if (/* 0x00 <= cp && */ cp <= 0x7f) {
        // 将 Unicode 码点转换为 UTF-8 编码，并存入结果字符串
        result.push_back(cp);
    }
    # 如果 Unicode 码点在 0x80 到 0x7ff 之间
    else if (0x80 <= cp && cp <= 0x7ff) {
        # 将 Unicode 码点转换成两个字节的 UTF-8 编码，并添加到结果数组中
        result.push_back(0xc0 | ((cp >> 6) & 0x1f));
        result.push_back(0x80 | (cp & 0x3f));
    }
    # 如果 Unicode 码点在 0x800 到 0xffff 之间
    else if (0x800 <= cp && cp <= 0xffff) {
        # 将 Unicode 码点转换成三个字节的 UTF-8 编码，并添加到结果数组中
    ...
    # 如果 Unicode 码点在 0x10000 到 0x10ffff 之间
        ...
        # 将 Unicode 码点转换成四个字节的 UTF-8 编码，并添加到结果数组中
...
static uint32_t codepoint_from_utf8(const std::string & utf8, size_t & offset) {
    ...
    // 如果是多字节字符
        ...
        // 抛出异常，因为不是有效的 UTF-8 字符
        throw std::invalid_argument("invalid character");
    }
    // 处理 2 字节字符
    else if (!(utf8[offset + 0] & 0x20)) {
        // 检查字符格式并解析出 Unicode 码点，更新偏移量
...
static std::vector<uint32_t> codepoints_from_utf8(const std::string & utf8) {
    ...
    size_t offset = 0;
    # 当偏移量小于 UTF-8 数据的大小时，执行循环
    while (offset < utf8.size()) {
        # 从 UTF-8 数据中获取一个代码点，并添加到结果数组中
...
static std::vector<uint16_t> codepoint_to_utf16(uint32_t cp) {
    ...
    // 如果码点在基本多文本平面（BMP）内（0x0000 到 0xffff）
    if (/* 0x0000 <= cp && */ cp <= 0xffff) {
        // 将码点直接添加到结果向量中
        result.emplace_back(cp);
    }
    // 如果码点在辅助平面（Supplementary Planes）内（0x10000 到 0x10ffff）
    else if (0x10000 <= cp && cp <= 0x10ffff) {
        // 计算辅助平面码点的 UTF-16 编码，并添加到结果向量中
        result.emplace_back(0xd800 | ((cp - 0x10000) >> 10));
        result.emplace_back(0xdc00 | ((cp - 0x10000) & 0x03ff));
    }
    // 如果码点不在上述范围内，抛出无效参数异常
    else {
        throw std::invalid_argument("invalid codepoint");
    }
    // 返回结果向量
...
static std::vector<uint16_t> codepoints_to_utf16(const std::vector<uint32_t> & cps) {
    ...
    // 遍历每个码点
    for (size_t i = 0; i < cps.size(); ++i) {
        // 调用 codepoint_to_utf16 函数将每个码点转换为 UTF-16 编码，并将结果添加到结果向量中
...
static uint32_t codepoint_from_utf16(const std::vector<uint16_t> & utf16, size_t & offset) {
    // 断言偏移量小于 UTF-16 编码的长度
    assert(offset < utf16.size());
    // 如果第一个 UTF-16 单元不是代理对的高位单元
    if (((utf16[0] >> 10) << 10) != 0xd800) {
        // 返回第一个 UTF-16 单元作为 Unicode 码点，并增加偏移量
        auto result = utf16[offset + 0];
        offset += 1;
        return result;
    }
    // 如果第一个 UTF-16 单元是代理对的高位单元
    else {
        // 如果下一个 UTF-16 单元不是代理对的低位单元，或者已经到达编码末尾，抛出无效字符异常
        if (offset + 1 >= utf16.size() || !((utf16[1] & 0xdc00) == 0xdc00))
            throw std::invalid_argument("invalid character");
        // 计算代理对的 Unicode 码点，并增加偏移量
        auto result = 0x10000 + (((utf16[0] & 0x03ff) << 10) | (utf16[1] & 0x03ff));
        offset += 2;
        return result;
    }
    // 如果无法解析出有效的 Unicode 码点，抛出无效字符串异常
    throw std::invalid_argument("invalid string");
}
...
static std::vector<uint32_t> codepoints_from_utf16(const std::vector<uint16_t> & utf16) {
    ...
    std::vector<uint32_t> result;
    size_t offset = 0;
    // 循环解析 UTF-16 编码中的每个 Unicode 码点，并添加到结果向量中
...
static std::unordered_map<uint32_t, int> codepoint_type_map() {
    # 创建一个无序映射，用于存储 Unicode 码点和它们的类型
    std::unordered_map<uint32_t, int> codepoint_types;
    # 遍历数字范围，将范围内的码点类型设置为数字
    for (auto p : digit_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_DIGIT;
    }
    # 遍历字母范围，将范围内的码点类型设置为字母
    for(auto p : letter_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_LETTER;
    }
    # 遍历空白字符范围，将范围内的码点类型设置为空白字符
    for(auto p : whitespace_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_WHITESPACE;
    }
    # 遍历重音符范围，将范围内的码点类型设置为重音符
    for(auto p : accent_mark_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_ACCENT_MARK;
    }
    # 遍历标点符号范围，将范围内的码点类型设置为标点符号
    for(auto p : punctuation_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_PUNCTUATION;
    }
    # 遍历符号范围，将范围内的码点类型设置为符号
    for (auto p : symbol_ranges) {
        for (auto i = p.first; i <= p.second; ++i)
            codepoint_types[i] = CODEPOINT_TYPE_SYMBOL;
    }
    # 遍历控制字符范围，将范围内的码点类型设置为控制字符
...
static int codepoint_type(uint32_t cp) {
    # 创建静态的 Unicode 代码点类型映射表
    static std::unordered_map<uint32_t, int> codepoint_types = codepoint_type_map();
    # 返回给定代码点的类型
    return codepoint_types[cp];
}

# 返回给定 UTF-8 字符串的代码点类型
static int codepoint_type(const std::string & utf8) {
    # 如果 UTF-8 字符串长度为 0，则返回未识别的代码点类型
    if (utf8.length() == 0)
