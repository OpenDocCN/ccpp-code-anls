# `nmap\zenmap\zenmapGUI\higwidgets\gtkutils.py`
...
def gtk_constant_name(group, value):
    ...
    """
    # 定义一个字典，将整数值映射到对应的 Gtk.ResponseType 常量字符串
    ...
    # 将 group_response 放入名为 'response' 的字典中
    groups = {'response': group_response}

    # 返回根据给定的 group 和 value 获取对应值的结果，如果找不到则返回错误信息
