# `nmap\libdnet-stripped\include\dnet\fw.h`
...
__BEGIN_DECLS
fw_t    *fw_open(void); // 打开防火墙
int     fw_add(fw_t *f, const struct fw_rule *rule); // 添加防火墙规则
int     fw_delete(fw_t *f, const struct fw_rule *rule); // 删除防火墙规则
int     fw_loop(fw_t *f, fw_handler callback, void *arg); // 循环处理防火墙规则
fw_t    *fw_close(fw_t *f); // 关闭防火墙
__END_DECLS
