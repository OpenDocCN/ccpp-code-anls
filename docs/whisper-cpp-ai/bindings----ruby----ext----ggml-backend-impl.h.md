# `whisper.cpp\bindings\ruby\ext\ggml-backend-impl.h`
...
extern "C" {
    ...
    // 定义 ggml_backend_i 结构体，包含了一系列函数指针，用于实现不同的功能
    struct ggml_backend_i {
        ...
        // 张量数据访问的函数指针
        // 这些函数可以是异步的，提供了用于同步访问的辅助函数，会自动调用 synchronize
    ...
    // 定义 ggml_backend 结构体，包含了 ggml_backend_i 接口和后端上下文
    struct ggml_backend {
