# `whisper.cpp\examples\talk-llama\unicode.h`
...
#include <unordered_map> // 包含无序映射容器的头文件
#include <vector> // 包含向量容器的头文件

static const std::vector<std::pair<uint32_t, uint32_t>> digit_ranges = { // 定义数字范围的静态常量向量
...
static const std::vector<std::pair<uint32_t, uint32_t>> letter_ranges = { // 定义字母范围的静态常量向量
...
# 定义一系列 Unicode 字符范围，每个范围由起始码点和结束码点组成
...
# 定义一系列 Unicode 字符范围，用大括号包围，每个范围由起始值和结束值组成
...
# 定义 Unicode 字符范围，每个范围由起始值和结束值组成
...
# 定义一系列范围，每个范围包含两个16进制数，表示Unicode字符的范围
...
# 定义一系列范围，每个范围包含两个16进制数，表示Unicode字符范围
...
# 定义一系列 Unicode 字符范围，每个范围由起始值和结束值组成
...
// 定义 Unicode 字符范围，每个元素为一对十六进制数，表示字符范围的起始和结束
{
...
// 定义标点符号 Unicode 范围，每个元素为一对十六进制数，表示标点符号范围的起始和结束
...
// 定义一组 Unicode 范围，每个范围由两个十六进制数表示
...
// 定义一组 Unicode 范围，每个范围由两个十六进制数表示，存储在 std::vector 中
...
// 定义一系列 Unicode 编码范围，用大括号包围，每个范围由起始编码和结束编码组成
{
...
// 定义一个静态函数，将 Unicode 编码转换为 UTF-8 编码
static std::string codepoint_to_utf8(uint32_t cp) {
    // 定义一个空字符串用于存储结果
    std::string result;
    // 如果 Unicode 编码在 0x00 到 0x7f 之间
    if (/* 0x00 <= cp && */ cp <= 0x7f) {
        // 将 Unicode 编码转换为对应的 UTF-8 编码并存储到结果字符串中
        result.push_back(cp);
    }
}
    # 如果 Unicode 码点在 0x80 到 0x7ff 之间
    else if (0x80 <= cp && cp <= 0x7ff) {
        # 将 Unicode 码点转换为两个字节的 UTF-8 编码并添加到结果中
        result.push_back(0xc0 | ((cp >> 6) & 0x1f));
        result.push_back(0x80 | (cp & 0x3f));
    }
    # 如果 Unicode 码点在 0x800 到 0xffff 之间
    else if (0x800 <= cp && cp <= 0xffff) {
        # 将 Unicode 码点转换为三个字节的 UTF-8 编码并添加到结果中
        result.push_back(0xe0 | ((cp >> 12) & 0x0f));
        result.push_back(0x80 | ((cp >> 6) & 0x3f));
        result.push_back(0x80 | (cp & 0x3f));
    }
    # 如果 Unicode 码点在 0x10000 到 0x10ffff 之间
    else if (0x10000 <= cp && cp <= 0x10ffff) {
        # 将 Unicode 码点转换为四个字节的 UTF-8 编码并添加到结果中
        result.push_back(0xf0 | ((cp >> 18) & 0x07));
        result.push_back(0x80 | ((cp >> 12) & 0x3f));
        result.push_back(0x80 | ((cp >> 6) & 0x3f));
        result.push_back(0x80 | (cp & 0x3f));
    }
    # 如果 Unicode 码点不在以上范围内
    else {
        # 抛出异常，表示无效的码点
...
// 将 Unicode 码点序列转换为 UTF-8 编码的字符串
static std::string codepoints_to_utf8(const std::vector<uint32_t> & cps) {
    // 初始化结果字符串
    std::string result;
    // 遍历 Unicode 码点序列，将每个码点转换为 UTF-8 编码并添加到结果字符串中
...
// 从 UTF-8 编码的字符串中解析出 Unicode 码点序列
static std::vector<uint32_t> codepoints_from_utf8(const std::string & utf8) {
    // 初始化结果码点序列
    std::vector<uint32_t> result;
    // 初始化偏移量
    size_t offset = 0;
    # 当偏移量小于 UTF-8 字符串的大小时，循环执行以下操作
    while (offset < utf8.size()) {
        # 从 UTF-8 字符串中获取一个 Unicode 码点，并将其添加到结果向量中
...
static std::unordered_map<uint32_t, int> codepoint_type_map() {
    // 创建一个无序映射，用于存储 Unicode 码点和对应的类型
    std::unordered_map<uint32_t, int> codepoint_types;
    // 遍历数字范围，将范围内的码点类型设为数字
    for (auto p : digit_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_DIGIT;
    }
    // 遍历字母范围，将范围内的码点类型设为字母
    for(auto p : letter_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_LETTER;
    }
    // 遍历空白字符范围，将范围内的码点类型设为空白字符
    for(auto p : whitespace_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_WHITESPACE;
    }
    // 遍历重音符范围，将范围内的码点类型设为重音符
    for(auto p : accent_mark_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_ACCENT_MARK;
    }
    // 遍历标点符号范围，将范围内的码点类型设为标点符号
    for(auto p : punctuation_ranges) {
        for(auto i = p.first; i <= p.second; ++ i)
            codepoint_types[i] = CODEPOINT_TYPE_PUNCTUATION;
    }
    // 遍历符号范围，将范围内的码点类型设为符号
    for (auto p : symbol_ranges) {
        for (auto i = p.first; i <= p.second; ++i)
            codepoint_types[i] = CODEPOINT_TYPE_SYMBOL;
    }
    // 遍历控制字符范围，将范围内的码点类型设为控制字符
...
// 返回字节到 Unicode 字符串的映射（Byte Pair Encoding）
static std::unordered_map<uint8_t, std::string> bytes_to_unicode_map_bpe() {
    // 创建空的映射
    std::unordered_map<uint8_t, std::string> map;
    // 遍历 ASCII 可见字符范围，将字符映射到对应的 UTF-8 字符串
    for (int ch = u'!'; ch <= u'~'; ++ch) {
        assert(0 <= ch && ch < 256);
        map[ch] = codepoint_to_utf8(ch);
    }
    // 遍历扩展 ASCII 字符范围，将字符映射到对应的 UTF-8 字符串
    for (int ch = u'¡'; ch <= u'¬'; ++ch) {
        assert(0 <= ch && ch < 256);
        map[ch] = codepoint_to_utf8(ch);
    }
