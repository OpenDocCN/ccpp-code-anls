...
#include "c_jh.h"  // 包含自定义的头文件 c_jh.h

#include <stdint.h>  // 包含标准整数类型的头文件
#include <string.h>  // 包含字符串处理函数的头文件

/*typedef unsigned long long uint64;*/
typedef uint64_t uint64;  // 定义 uint64_t 为 uint64 类型

/*define data alignment for different C compilers*/
#if defined(__GNUC__)
      #define DATA_ALIGN16(x) x __attribute__ ((aligned(16)))  // 根据不同的 C 编译器定义数据对齐方式
#else
      #define DATA_ALIGN16(x) __declspec(align(16)) x
#endif

typedef struct {
    int hashbitlen;                         /*the message digest size*/  // 消息摘要的大小
    unsigned long long databitlen;    /*the message size in bits*/  // 消息的位大小
    unsigned long long datasize_in_buffer;      /*the size of the message remained in buffer; assumed to be multiple of 8bits except for the last partial block at the end of the message*/  // 缓冲区中剩余消息的大小；假设是 8 位的倍数，除了消息末尾的最后一个部分块
    DATA_ALIGN16(uint64 x[8][2]);     /*the 1024-bit state, ( x[i][0] || x[i][1] ) is the ith row of the state in the pseudocode*/  // 1024 位状态，( x[i][0] || x[i][1] ) 是伪代码中状态的第 i 行
    unsigned char buffer[64];         /*the 512-bit message block to be hashed;*/  // 要进行哈希的 512 位消息块
} hashState;

/*The initial hash value H(0)*/
# 定义了一个名为 JH224_H0 的常量，存储了128个无符号字符
...
# 定义了一个名为 JH256_H0 的常量，存储了128个无符号字符
...
/* E8_bitslice_roundconstant常量数组，用于某种加密算法 */
...
/* 定义静态函数 E8，接受 hashState 指针参数 */
static void E8(hashState *state);  /*The bijective function E8, in bitslice form*/
/* 定义静态函数 F8，接受 hashState 指针参数 */
static void F8(hashState *state);  /*The compression function F8 */

/* 定义初始化哈希状态的函数 Init，接受 hashState 指针和哈希位长度参数 */
static HashReturn Init(hashState *state, int hashbitlen);
/* 定义更新哈希状态的函数 Update，接受 hashState 指针、数据指针和数据位长度参数 */
static HashReturn Update(hashState *state, const BitSequence *data, DataLength databitlen);
/* 定义结束哈希状态的函数 Final，接受 hashState 指针、哈希值指针参数 */
static HashReturn Final(hashState *state, BitSequence *hashval);
/* 定义哈希函数 jh_hash，接受哈希位长度、数据指针、数据位长度和哈希值指针参数 */
HashReturn jh_hash(int hashbitlen, const BitSequence *data,DataLength databitlen, BitSequence *hashval);

/* 定义宏 SWAP1，用于交换 64 位 x 中的位 2i 和 2i+1 */
#define SWAP1(x)   (x) = ((((x) & 0x5555555555555555ULL) << 1) | (((x) & 0xaaaaaaaaaaaaaaaaULL) >> 1));
/* 定义宏 SWAP2，用于交换 64 位 x 中的位 4i||4i+1 和 4i+2||4i+3 */
#define SWAP2(x)   (x) = ((((x) & 0x3333333333333333ULL) << 2) | (((x) & 0xccccccccccccccccULL) >> 2));
/* 定义宏 SWAP4，用于交换 64 位 x 中的位 8i||8i+1||8i+2||8i+3 和 8i+4||8i+5||8i+6||8i+7 */
#define SWAP4(x)   (x) = ((((x) & 0x0f0f0f0f0f0f0f0fULL) << 4) | (((x) & 0xf0f0f0f0f0f0f0f0ULL) >> 4));
/* 定义宏 SWAP8，用于交换 64 位 x 中的位 16i||16i+1||......||16i+7 和 16i+8||16i+9||......||16i+15 */
#define SWAP8(x)   (x) = ((((x) & 0x00ff00ff00ff00ffULL) << 8) | (((x) & 0xff00ff00ff00ff00ULL) >> 8));
/* 定义宏 SWAP16，用于交换 64 位 x 中的位 32i||32i+1||......||32i+15 和 32i+16||32i+17||......||32i+31 */
#define SWAP16(x)  (x) = ((((x) & 0x0000ffff0000ffffULL) << 16) | (((x) & 0xffff0000ffff0000ULL) >> 16));
/* 定义宏 SWAP32，用于交换 64 位 x 中的位 64i||64i+1||......||64i+31 和 64i+32||64i+33||......||64i+63 */
# 定义一个宏，用于交换32位整数的高低位
#define SWAP32(x)  (x) = (((x) << 32) | ((x) >> 32));

# 定义MDS变换，对8个32位整数进行操作
...
# 定义S盒变换，对8个32位整数进行操作，cc0和cc1是常量
...
# 定义E8函数，对hashState结构体进行操作
static void E8(hashState *state)
}

# 定义F8函数，对hashState结构体进行操作
static void F8(hashState *state)
{
      // 将状态中的 x 指针转换为 uint64_t 类型的指针
      uint64_t* x = (uint64_t*)state->x;
      // 将状态中的 buffer 指针转换为 uint64_t 类型的指针
      const uint64_t* buf = (uint64*)state->buffer;

      /*用 1024 位哈希状态的前半部分与 512 位消息进行异或运算*/
      for (int i = 0; i < 8; ++i) x[i] ^= buf[i];

      /*双射函数 E8 */
      E8(state);

      /*用 1024 位哈希状态的后半部分与 512 位消息进行异或运算*/
      for (int i = 0; i < 8; ++i) x[i + 8] ^= buf[i];
}

/*在对消息进行哈希之前，将哈希状态初始化为 H0 */
static HashReturn Init(hashState *state, int hashbitlen)
{
      // 数据位长度初始化为 0
      state->databitlen = 0;
...
/*对每个 512 位消息块进行哈希，除了最后一个部分块*/
static HashReturn Update(hashState *state, const BitSequence *data, DataLength databitlen)
}

/*填充消息，处理填充块，将哈希值 H 截断以获得消息摘要*/
static HashReturn Final(hashState *state, BitSequence *hashval)
}

/* 哈希一个消息，
   三个输入：消息摘要大小（以位为单位）（hashbitlen）；消息（data）；消息长度（以位为单位）（databitlen）
   一个输出：消息摘要（hashval）
*/
HashReturn jh_hash(int hashbitlen, const BitSequence *data,DataLength databitlen, BitSequence *hashval)
# 定义哈希状态变量
...
# 如果哈希位长度为224、256、384或512，则执行以下操作
if ( hashbitlen == 224 || hashbitlen == 256 || hashbitlen == 384 || hashbitlen == 512 ) {
    # 初始化哈希状态
    Init(&state, hashbitlen);
    # 更新哈希状态，传入数据和数据位长度
    Update(&state, data, databitlen);
    # 完成哈希计算，将结果存储在hashval中
    Final(&state, hashval);
    # 返回成功状态
    return SUCCESS;
}
# 如果哈希位长度不符合要求，则返回错误状态
else
    return(BAD_HASHLEN);
```