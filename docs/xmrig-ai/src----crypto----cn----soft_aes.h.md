# `xmrig\src\crypto\cn\soft_aes.h`
...
static inline __m128i soft_aesenc(__m128i in, __m128i key)
{
    ...
    # 使用 saes_table 中的值进行按位异或操作，生成 out 变量
    __m128i out = _mm_set_epi32(
        (saes_table[0][x3 & 0xff] ^ saes_table[1][(x0 >> 8) & 0xff] ^ saes_table[2][(x1 >> 16) & 0xff] ^ saes_table[3][x2 >> 24]),
        (saes_table[0][x2 & 0xff] ^ saes_table[1][(x3 >> 8) & 0xff] ^ saes_table[2][(x0 >> 16) & 0xff] ^ saes_table[3][x1 >> 24]),
        (saes_table[0][x1 & 0xff] ^ saes_table[1][(x2 >> 8) & 0xff] ^ saes_table[2][(x3 >> 16) & 0xff] ^ saes_table[3][x0 >> 24]),
    ...
    # 对 out 变量和 key 变量进行按位异或操作，并返回结果
    return _mm_xor_si128(out, key);
// 定义一个静态内联函数，用于对输入的32位无符号整数进行字节替换操作
static inline uint32_t sub_word(uint32_t key)
{
    return (saes_sbox[key >> 24 ] << 24)   |  // 将输入的32位整数按字节进行替换，并按位进行合并
        (saes_sbox[(key >> 16) & 0xff] << 16 ) |  // 将输入的32位整数按字节进行替换，并按位进行合并
        (saes_sbox[(key >> 8)  & 0xff] << 8  ) |  // 将输入的32位整数按字节进行替换，并按位进行合并
         saes_sbox[key & 0xff];  // 将输入的32位整数按字节进行替换
}

// 如果没有定义 HAVE_ROTR，则定义一个静态内联函数，用于对输入的32位无符号整数进行循环右移操作
#ifndef HAVE_ROTR
static inline uint32_t _rotr(uint32_t value, uint32_t amount)
{
    return (value >> amount) | (value << ((32 - amount) & 31));  // 对输入的32位整数进行循环右移操作
}
#endif

// 定义一个模板函数，用于生成软件实现的 AES 密钥扩展辅助函数
template<uint8_t rcon>
static inline __m128i soft_aeskeygenassist(__m128i key)
{
    // 计算两个32位整数 X1 和 X3，分别对输入的128位整数进行字节替换操作
    ...
    // 返回一个128位整数，其中包含对 X1 和 X3 进行循环右移和异或操作得到的结果
