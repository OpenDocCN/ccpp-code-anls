...
// 定义了一个包含256个元素的常量数组，每个元素都是64位的无符号整数
static const sph_u64 plain_T0[256] = {
    ...
    SPH_C64(0x3C677381BDCEBDBD),  // 将十六进制数转换为64位整数
    SPH_C64(0x8FBA34D25D695D5D),  // 将十六进制数转换为64位整数
    SPH_C64(0x9020508010401010),  // 将十六进制数转换为64位整数
    SPH_C64(0x07F503F3F4F7F4F4),  // 将十六进制数转换为64位整数
    SPH_C64(0xDD8BC016CB0BCBCB),  // 将十六进制数转换为64位整数
    SPH_C64(0xD37CC6ED3EF83E3E),  // 将十六进制数转换为64位整数
    SPH_C64(0x2D0A112805140505),  // 将十六进制数转换为64位整数
    SPH_C64(0x78CEE61F67816767),  // 将十六进制数转换为64位整数
    SPH_C64(0x97D55373E4B7E4E4),  // 将十六进制数转换为64位整数
    SPH_C64(0x024EBB25279C2727),  // 将十六进制数转换为64位整数
    SPH_C64(0x7382583241194141),  // 将十六进制数转换为64位整数
    SPH_C64(0xA70B9D2C8B168B8B),  // 将十六进制数转换为64位整数
    SPH_C64(0xF6530151A7A6A7A7),  // 将十六进制数转换为64位整数
    SPH_C64(0xB2FA94CF7DE97D7D),  // 将十六进制数转换为64位整数
    SPH_C64(0x4937FBDC956E9595),  // 将十六进制数转换为64位整数
    SPH_C64(0x56AD9F8ED847D8D8),  // 将十六进制数转换为64位整数
    SPH_C64(0x70EB308BFBCBFBFB),  // 将十六进制数转换为64位整数
    SPH_C64(0xCDC17123EE9FEEEE),  // 将十六进制数转换为64位整数
    SPH_C64(0xBBF891C77CED7C7C),  // 将十六进制数转换为64位整数
    SPH_C64(0x71CCE31766856666),  // 将十六进制数转换为64位整数
    SPH_C64(0x7BA78EA6DD53DDDD),  // 将十六进制数转换为64位整数
    SPH_C64(0xAF2E4BB8175C1717),  // 将十六进制数转换为64位整数
    SPH_C64(0x458E460247014747),  // 将十六进制数转换为64位整数
    SPH_C64(0x1A21DC849E429E9E),  // 将十六进制数转换为64位整数
    SPH_C64(0xD489C51ECA0FCACA),  // 将十六进制数转换为64位整数
    SPH_C64(0x585A99752DB42D2D),  // 将十六进制数转换为64位整数
    SPH_C64(0x2E637991BFC6BFBF),  // 将十六进制数转换为64位整数
    SPH_C64(0x3F0E1B38071C0707),  // 将十六进制数转换为64位整数
    SPH_C64(0xAC472301AD8EADAD),  // 将十六进制数转换为64位整数
    SPH_C64(0xB0B42FEA5A755A5A),  // 将十六进制数转换为64位整数
    SPH_C64(0xEF1BB56C83368383),  // 将十六进制数转换为64位整数
    SPH_C64(0xB666FF8533CC3333),  // 将十六进制数转换为64位整数
    SPH_C64(0x5CC6F23F63916363),  // 将十六进制数转换为64位整数
    SPH_C64(0x12040A1002080202),  // 将十六进制数转换为64位整数
    SPH_C64(0x93493839AA92AAAA),  // 将十六进制数转换为64位整数
    SPH_C64(0xDEE2A8AF71D97171),  // 将十六进制数转换为64位整数
    SPH_C64(0xC68DCF0EC807C8C8),  // 将十六进制数转换为64位整数
    SPH_C64(0xD1327DC819641919),  // 将十六进制数转换为64位整数
    SPH_C64(0x3B92707249394949),  // 将十六进制数转换为64位整数
    SPH_C64(0x5FAF9A86D943D9D9),  // 将十六进制数转换为64位整数
    SPH_C64(0x31F91DC3F2EFF2F2),  // 将十六进制数转换为64位整数
    SPH_C64(0xA8DB484BE3ABE3E3),  // 将十六进制数转换为64位整数
    SPH_C64(0xB9B62AE25B715B5B),  // 将十六进制数转换为64位整数
    SPH_C64(0xBC0D9234881A8888),  // 将十六进制数转换为64位整数
    SPH_C64(0x3E29C8A49A529A9A),  // 将十六进制数转换为64位整数
    SPH_C64(0x0B4CBE2D26982626),  // 将十六进制数转换为64位整数
    SPH_C64(0xBF64FA8D32C83232),  // 将十六进制数转换为64位整数
    SPH_C64(0x597D4AE9B0FAB0B0),  // 将十六进制数转换为64位整数
    SPH_C64(0xF2CF6A1BE983E9E9),  // 将十六进制数转换为64位整数
    SPH_C64(0x771E33780F3C0F0F),  // 将十六进制数转换为64位整数
    SPH_C64(0x33B7A6E6D573D5D5),  // 将十六进制数转换为64位整数
    SPH_C64(0xF41DBA74803A8080),  // 将十六进制数转换为64位整数
    SPH_C64(0x27617C99BEC2BEBE),  // 将十六进制数转换为64位整数
    SPH_C64(0xEB87DE26CD13CDCD),  // 将十六进制数转换为64位整数
    SPH_C64(0x8968E4BD34D03434),  // 将十六进制数转换为64位整数
    SPH_C64(0x3290757A483D4848),  // 将十六进制数转换为64位整数
    SPH_C64(0x54E324ABFFDBFFFF),  // 将十六进制数转换为64位整数
    SPH_C64(0x8DF48FF77AF57A7A),  // 将十六进制数转换为64位整数
    SPH_C64(0x643DEAF4907A9090),  // 将十六进制数转换为64位整数
    SPH_C64(0x9DBE3EC25F615F5F),  // 将十六进制数转换为64位整数
    SPH_C64(0x3D40A01D20802020),  // 将十六进制数转换为64位整数
    SPH_C64(0x0FD0D56768BD6868),  // 将十六进制数转换为64位整数
    SPH_C64(0xCA3472D01A681A1A),  // 将十六进制数转换为64位整数
    SPH_C64(0xB7412C19AE82AEAE),  // 将十六进制数转换为64位整数
    SPH_C64(0x7D755EC9B4EAB4B4),  // 用常量生成64位整数
    SPH_C64(0xCEA8199A544D5454),  // 用常量生成64位整数
    SPH_C64(0x7F3BE5EC93769393),  // 用常量生成64位整数
    SPH_C64(0x2F44AA0D22882222),  // 用常量生成64位整数
    SPH_C64(0x63C8E907648D6464),  // 用常量生成64位整数
    SPH_C64(0x2AFF12DBF1E3F1F1),  // 用常量生成64位整数
    SPH_C64(0xCCE6A2BF73D17373),  // 用常量生成64位整数
    SPH_C64(0x82245A9012481212),  // 用常量生成64位整数
    SPH_C64(0x7A805D3A401D4040),  // 用常量生成64位整数
    SPH_C64(0x4810284008200808),  // 用常量生成64位整数
    SPH_C64(0x959BE856C32BC3C3),  // 用常量生成64位整数
    SPH_C64(0xDFC57B33EC97ECEC),  // 用常量生成64位整数
    SPH_C64(0x4DAB9096DB4BDBDB),  // 用常量生成64位整数
    SPH_C64(0xC05F1F61A1BEA1A1),  // 用常量生成64位整数
    SPH_C64(0x9107831C8D0E8D8D),  // 用常量生成64位整数
    SPH_C64(0xC87AC9F53DF43D3D),  // 用常量生成64位整数
    SPH_C64(0x5B33F1CC97669797),  // 用常量生成64位整数
    SPH_C64(0x0000000000000000),  // 用常量生成64位整数
    SPH_C64(0xF983D436CF1BCFCF),  // 用常量生成64位整数
    SPH_C64(0x6E5687452BAC2B2B),  // 用常量生成64位整数
    SPH_C64(0xE1ECB39776C57676),  // 用常量生成64位整数
    SPH_C64(0xE619B06482328282),  // 用常量生成64位整数
    SPH_C64(0x28B1A9FED67FD6D6),  // 用常量生成64位整数
    SPH_C64(0xC33677D81B6C1B1B),  // 用常量生成64位整数
    SPH_C64(0x74775BC1B5EEB5B5),  // 用常量生成64位整数
    SPH_C64(0xBE432911AF86AFAF),  // 用常量生成64位整数
    SPH_C64(0x1DD4DF776AB56A6A),  // 用常量生成64位整数
    SPH_C64(0xEAA00DBA505D5050),  // 用常量生成64位整数
    SPH_C64(0x578A4C1245094545),  // 用常量生成64位整数
    SPH_C64(0x38FB18CBF3EBF3F3),  // 用常量生成64位整数
    SPH_C64(0xAD60F09D30C03030),  // 用常量生成64位整数
    SPH_C64(0xC4C3742BEF9BEFEF),  // 用常量生成64位整数
    SPH_C64(0xDA7EC3E53FFC3F3F),  // 用常量生成64位整数
    SPH_C64(0xC7AA1C9255495555),  // 用常量生成64位整数
    SPH_C64(0xDB591079A2B2A2A2),  // 用常量生成64位整数
    SPH_C64(0xE9C96503EA8FEAEA),  // 用常量生成64位整数
    SPH_C64(0x6ACAEC0F65896565),  // 用常量生成64位整数
    SPH_C64(0x036968B9BAD2BABA),  // 用常量生成64位整数
    SPH_C64(0x4A5E93652FBC2F2F),  // 用常量生成64位整数
    SPH_C64(0x8E9DE74EC027C0C0),  // 用常量生成64位整数
    SPH_C64(0x60A181BEDE5FDEDE),  // 用常量生成64位整数
    SPH_C64(0xFC386CE01C701C1C),  // 用常量生成64位整数
    SPH_C64(0x46E72EBBFDD3FDFD),  // 用常量生成64位整数
    SPH_C64(0x1F9A64524D294D4D),  // 用常量生成64位整数
    SPH_C64(0x7639E0E492729292),  // 用常量生成64位整数
    SPH_C64(0xFAEABC8F75C97575),  // 用常量生成64位整数
    SPH_C64(0x360C1E3006180606),  // 用常量生成64位整数
    SPH_C64(0xAE0998248A128A8A),  // 用常量生成64位整数
    SPH_C64(0x4B7940F9B2F2B2B2),  // 用常量生成64位整数
    SPH_C64(0x85D15963E6BFE6E6),  // 用常量生成64位整数
    SPH_C64(0x7E1C36700E380E0E),  // 用常量生成64位整数
    SPH_C64(0xE73E63F81F7C1F1F),  // 用常量生成64位整数
    SPH_C64(0x55C4F73762956262),  // 用常量生成64位整数
    SPH_C64(0x3AB5A3EED477D4D4),  // 用常量生成64位整数
    SPH_C64(0x814D3229A89AA8A8),  // 用常量生成64位整数
    SPH_C64(0x5231F4C496629696),  // 用常量生成64位整数
    SPH_C64(0x62EF3A9BF9C3F9F9),  // 用常量生成64位整数
    SPH_C64(0xA397F666C533C5C5),  // 用常量生成64位整数
    SPH_C64(0x104AB13525942525),  // 用常量生成64位整数
    SPH_C64(0xABB220F259795959),  // 用常量生成64位整数
    SPH_C64(0xD015AE54842A8484),  // 用常量生成64位整数
    SPH_C64(0xC5E4A7B772D57272),  // 用常量生成64位整数
    SPH_C64(0xEC72DDD539E43939),  // 用常量生成64位整数
    SPH_C64(0x1698615A4C2D4C4C),  // 用常量生成64位整数
    # 定义一系列64位常量，使用SPH_C64函数将十六进制数转换为64位整数
    ...
    # 定义一系列64位的常量，使用SPH_C64函数将十六进制数转换为64位整数
...
# 如果不是小内存占用的 Whirlpool 算法，则定义一个 256 个元素的常量数组 plain_T1
...
static const sph_u64 plain_T1[256] = {
    ...
    SPH_C64(0xAB9096DB4BDBDB4D),  // 用于计算哈希的常量
    SPH_C64(0x5F1F61A1BEA1A1C0),  // 用于计算哈希的常量
    SPH_C64(0x07831C8D0E8D8D91),  // 用于计算哈希的常量
    SPH_C64(0x7AC9F53DF43D3DC8),  // 用于计算哈希的常量
    SPH_C64(0x33F1CC976697975B),  // 用于计算哈希的常量
    SPH_C64(0x0000000000000000),  // 用于计算哈希的常量
    SPH_C64(0x83D436CF1BCFCFF9),  // 用于计算哈希的常量
    SPH_C64(0x5687452BAC2B2B6E),  // 用于计算哈希的常量
    SPH_C64(0xECB39776C57676E1),  // 用于计算哈希的常量
    SPH_C64(0x19B06482328282E6),  // 用于计算哈希的常量
    SPH_C64(0xB1A9FED67FD6D628),  // 用于计算哈希的常量
    SPH_C64(0x3677D81B6C1B1BC3),  // 用于计算哈希的常量
    SPH_C64(0x775BC1B5EEB5B574),  // 用于计算哈希的常量
    SPH_C64(0x432911AF86AFAFBE),  // 用于计算哈希的常量
    SPH_C64(0xD4DF776AB56A6A1D),  // 用于计算哈希的常量
    SPH_C64(0xA00DBA505D5050EA),  // 用于计算哈希的常量
    SPH_C64(0x8A4C124509454557),  // 用于计算哈希的常量
    SPH_C64(0xFB18CBF3EBF3F338),  // 用于计算哈希的常量
    SPH_C64(0x60F09D30C03030AD),  // 用于计算哈希的常量
    SPH_C64(0xC3742BEF9BEFEFC4),  // 用于计算哈希的常量
    SPH_C64(0x7EC3E53FFC3F3FDA),  // 用于计算哈希的常量
    SPH_C64(0xAA1C9255495555C7),  // 用于计算哈希的常量
    SPH_C64(0x591079A2B2A2A2DB),  // 用于计算哈希的常量
    SPH_C64(0xC96503EA8FEAEAE9),  // 用于计算哈希的常量
    SPH_C64(0xCAEC0F658965656A),  // 用于计算哈希的常量
    SPH_C64(0x6968B9BAD2BABA03),  // 用于计算哈希的常量
    SPH_C64(0x5E93652FBC2F2F4A),  // 用于计算哈希的常量
    SPH_C64(0x9DE74EC027C0C08E),  // 用于计算哈希的常量
    SPH_C64(0xA181BEDE5FDEDE60),  // 用于计算哈希的常量
    SPH_C64(0x386CE01C701C1CFC),  // 用于计算哈希的常量
    SPH_C64(0xE72EBBFDD3FDFD46),  // 用于计算哈希的常量
    SPH_C64(0x9A64524D294D4D1F),  // 用于计算哈希的常量
    SPH_C64(0x39E0E49272929276),  // 用于计算哈希的常量
    SPH_C64(0xEABC8F75C97575FA),  // 用于计算哈希的常量
    SPH_C64(0x0C1E300618060636),  // 用于计算哈希的常量
    SPH_C64(0x0998248A128A8AAE),  // 用于计算哈希的常量
    SPH_C64(0x7940F9B2F2B2B24B),  // 用于计算哈希的常量
    SPH_C64(0xD15963E6BFE6E685),  // 用于计算哈希的常量
    SPH_C64(0x1C36700E380E0E7E),  // 用于计算哈希的常量
    SPH_C64(0x3E63F81F7C1F1FE7),  // 用于计算哈希的常量
    SPH_C64(0xC4F7376295626255),  // 用于计算哈希的常量
    SPH_C64(0xB5A3EED477D4D43A),  // 用于计算哈希的常量
    SPH_C64(0x4D3229A89AA8A881),  // 用于计算哈希的常量
    SPH_C64(0x31F4C49662969652),  // 用于计算哈希的常量
    SPH_C64(0xEF3A9BF9C3F9F962),  // 用于计算哈希的常量
    SPH_C64(0x97F666C533C5C5A3),  // 用于计算哈希的常量
    SPH_C64(0x4AB1352594252510),  // 用于计算哈希的常量
    SPH_C64(0xB220F259795959AB),  // 用于计算哈希的常量
    SPH_C64(0x15AE54842A8484D0),  // 用于计算哈希的常量
    SPH_C64(0xE4A7B772D57272C5),  // 用于计算哈希的常量
    SPH_C64(0x72DDD539E43939EC),  // 用于计算哈希的常量
    SPH_C64(0x98615A4C2D4C4C16),  // 用于计算哈希的常量
    SPH_C64(0xBC3BCA5E655E5E94),  // 用于计算哈希的常量
    SPH_C64(0xF085E778FD78789F),  // 用于计算哈希的常量
    SPH_C64(0x70D8DD38E03838E5),  // 用于计算哈希的常量
    SPH_C64(0x0586148C0A8C8C98),  // 用于计算哈希的常量
    SPH_C64(0xBFB2C6D163D1D117),  // 用于计算哈希的常量
    SPH_C64(0x570B41A5AEA5A5E4),  // 用于计算哈希的常量
    SPH_C64(0xD94D43E2AFE2E2A1),  // 用于计算哈希的常量
    SPH_C64(0xC2F82F619961614E),  // 用于计算哈希的常量
    SPH_C64(0x7B45F1B3F6B3B342),  // 用于计算哈希的常量
    SPH_C64(0x42A5152184212134),  // 用于计算哈希的常量
    SPH_C64(0x25D6949C4A9C9C08),  // 用于计算哈希的常量
    SPH_C64(0x3C66F01E781E1EEE),  // 用于计算哈希的常量
    # 定义一系列64位的常量，使用SPH_C64函数进行转换
...
static const sph_u64 plain_T3[256] = {
    ...
    # 这是一个函数调用，传入两个参数
    # 参数1: 0x93F8C7F8F86BED3F，使用SPH_C64函数对其进行处理
    # 参数2: 0x4486228686C211A4，使用SPH_C64函数对其进行处理
# 定义一个静态的64位无符号整数数组，用于存储常量
static const sph_u64 plain_T4[256] = {
    # 下面是256个64位无符号整数的十六进制表示，每个整数占8个字节
    # 这些整数是用于加密算法的常量
    ...
    # 定义了两个常量，使用了SPH_C64函数对十六进制数进行了转换
    SPH_C64(0xF8C7F8F86BED3F93), SPH_C64(0x86228686C211A444)
# 定义一个名为 plain_T5 的静态常量数组，包含 256 个 64 位的十六进制数
static const sph_u64 plain_T5[256] = {
    # 以下为 256 个 64 位的十六进制数，每两个数字为一组，表示一个 64 位的数值
    ...
    # 调用SPH_C64函数，传入参数0xC7F8F86BED3F93F8，并将结果作为参数传入SPH_C64函数中
    SPH_C64(0xC7F8F86BED3F93F8), 
    # 调用SPH_C64函数，传入参数0x228686C211A44486，并将结果作为参数传入SPH_C64函数中
    SPH_C64(0x228686C211A44486)
# 定义一个静态的64位无符号整数数组，包含256个元素
static const sph_u64 plain_T6[256] = {
    # 以下是256个64位无符号整数的十六进制表示
    ...
    SPH_C64(0x8D8D9107831C8D0E),  // 将十六进制数转换为64位整数
    SPH_C64(0x3D3DC87AC9F53DF4),
    SPH_C64(0x97975B33F1CC9766),
    SPH_C64(0x0000000000000000),
    SPH_C64(0xCFCFF983D436CF1B),
    SPH_C64(0x2B2B6E5687452BAC),
    SPH_C64(0x7676E1ECB39776C5),
    SPH_C64(0x8282E619B0648232),
    SPH_C64(0xD6D628B1A9FED67F),
    SPH_C64(0x1B1BC33677D81B6C),
    SPH_C64(0xB5B574775BC1B5EE),
    SPH_C64(0xAFAFBE432911AF86),
    SPH_C64(0x6A6A1DD4DF776AB5),
    SPH_C64(0x5050EAA00DBA505D),
    SPH_C64(0x4545578A4C124509),
    SPH_C64(0xF3F338FB18CBF3EB),
    SPH_C64(0x3030AD60F09D30C0),
    SPH_C64(0xEFEFC4C3742BEF9B),
    SPH_C64(0x3F3FDA7EC3E53FFC),
    SPH_C64(0x5555C7AA1C925549),
    SPH_C64(0xA2A2DB591079A2B2),
    SPH_C64(0xEAEAE9C96503EA8F),
    SPH_C64(0x65656ACAEC0F6589),
    SPH_C64(0xBABA036968B9BAD2),
    SPH_C64(0x2F2F4A5E93652FBC),
    SPH_C64(0xC0C08E9DE74EC027),
    SPH_C64(0xDEDE60A181BEDE5F),
    SPH_C64(0x1C1CFC386CE01C70),
    SPH_C64(0xFDFD46E72EBBFDD3),
    SPH_C64(0x4D4D1F9A64524D29),
    SPH_C64(0x92927639E0E49272),
    SPH_C64(0x7575FAEABC8F75C9),
    SPH_C64(0x0606360C1E300618),
    SPH_C64(0x8A8AAE0998248A12),
    SPH_C64(0xB2B24B7940F9B2F2),
    SPH_C64(0xE6E685D15963E6BF),
    SPH_C64(0x0E0E7E1C36700E38),
    SPH_C64(0x1F1FE73E63F81F7C),
    SPH_C64(0x626255C4F7376295),
    SPH_C64(0xD4D43AB5A3EED477),
    SPH_C64(0xA8A8814D3229A89A),
    SPH_C64(0x96965231F4C49662),
    SPH_C64(0xF9F962EF3A9BF9C3),
    SPH_C64(0xC5C5A397F666C533),
    SPH_C64(0x2525104AB1352594),
    SPH_C64(0x5959ABB220F25979),
    SPH_C64(0x8484D015AE54842A),
    SPH_C64(0x7272C5E4A7B772D5),
    SPH_C64(0x3939EC72DDD539E4),
    SPH_C64(0x4C4C1698615A4C2D),
    SPH_C64(0x5E5E94BC3BCA5E65),
    SPH_C64(0x78789FF085E778FD),
    SPH_C64(0x3838E570D8DD38E0),
    SPH_C64(0x8C8C980586148C0A),
    SPH_C64(0xD1D117BFB2C6D163),
    SPH_C64(0xA5A5E4570B41A5AE),
    SPH_C64(0xE2E2A1D94D43E2AF),
    SPH_C64(0x61614EC2F82F6199),
    SPH_C64(0xB3B3427B45F1B3F6),
    SPH_C64(0x21213442A5152184),
    SPH_C64(0x9C9C0825D6949C4A),
    SPH_C64(0x1E1EEE3C66F01E78),
    SPH_C64(0x4343618652224311),
    SPH_C64(0xC7C7B193FC76C73B),
    # 定义一个包含64位常量的列表
...
注释：这段代码定义了一个静态常量数组`plain_T7`，包含了256个64位的无符号整数。这些整数是预定义的常量值，用于后续的计算和操作。
    # 定义一个包含64位常量的列表
    ...
    SPH_C64(0xFC4FE52BB3FCD7FC),  // 用于计算哈希的常量
    SPH_C64(0x0424081420041004),  // 用于计算哈希的常量
    SPH_C64(0x51E3A208B2515951),  // 用于计算哈希的常量
    SPH_C64(0x99252FC7BC995E99),  // 用于计算哈希的常量
    SPH_C64(0x6D22DAC44F6DA96D),  // 用于计算哈希的常量
    SPH_C64(0x0D651A39680D340D),  // 用于计算哈希的常量
    SPH_C64(0xFA79E93583FACFFA),  // 用于计算哈希的常量
    SPH_C64(0xDF69A384B6DF5BDF),  // 用于计算哈希的常量
    SPH_C64(0x7EA9FC9BD77EE57E),  // 用于计算哈希的常量
    SPH_C64(0x241948B43D249024),  // 用于计算哈希的常量
    SPH_C64(0x3BFE76D7C53BEC3B),  // 用于计算哈希的常量
    SPH_C64(0xAB9A4B3D31AB96AB),  // 用于计算哈希的常量
    SPH_C64(0xCEF081D13ECE1FCE),  // 用于计算哈希的常量
    SPH_C64(0x1199225588114411),  // 用于计算哈希的常量
    SPH_C64(0x8F8303890C8F068F),  // 用于计算哈希的常量
    SPH_C64(0x4E049C6B4A4E254E),  // 用于计算哈希的常量
    SPH_C64(0xB7667351D1B7E6B7),  // 用于计算哈希的常量
    SPH_C64(0xEBE0CB600BEB8BEB),  // 用于计算哈希的常量
    SPH_C64(0x3CC178CCFD3CF03C),  // 用于计算哈希的常量
    SPH_C64(0x81FD1FBF7C813E81),  // 用于计算哈希的常量
    SPH_C64(0x944035FED4946A94),  // 用于计算哈希的常量
    SPH_C64(0xF71CF30CEBF7FBF7),  // 用于计算哈希的常量
    SPH_C64(0xB9186F67A1B9DEB9),  // 用于计算哈希的常量
    SPH_C64(0x138B265F98134C13),  // 用于计算哈希的常量
    SPH_C64(0x2C51589C7D2CB02C),  // 用于计算哈希的常量
    SPH_C64(0xD305BBB8D6D36BD3),  // 用于计算哈希的常量
    SPH_C64(0xE78CD35C6BE7BBE7),  // 用于计算哈希的常量
    SPH_C64(0x6E39DCCB576EA56E),  // 用于计算哈希的常量
    SPH_C64(0xC4AA95F36EC437C4),  // 用于计算哈希的常量
    SPH_C64(0x031B060F18030C03),  // 用于计算哈希的常量
    SPH_C64(0x56DCAC138A564556),  // 用于计算哈希的常量
    SPH_C64(0x445E88491A440D44),  // 用于计算哈希的常量
    SPH_C64(0x7FA0FE9EDF7FE17F),  // 用于计算哈希的常量
    SPH_C64(0xA9884F3721A99EA9),  // 用于计算哈希的常量
    SPH_C64(0x2A6754824D2AA82A),  // 用于计算哈希的常量
    SPH_C64(0xBB0A6B6DB1BBD6BB),  // 用于计算哈希的常量
    SPH_C64(0xC1879FE246C123C1),  // 用于计算哈希的常量
    SPH_C64(0x53F1A602A2535153),  // 用于计算哈希的常量
    SPH_C64(0xDC72A58BAEDC57DC),  // 用于计算哈希的常量
    SPH_C64(0x0B531627580B2C0B),  // 用于计算哈希的常量
    SPH_C64(0x9D0127D39C9D4E9D),  // 用于计算哈希的常量
    SPH_C64(0x6C2BD8C1476CAD6C),  // 用于计算哈希的常量
    SPH_C64(0x31A462F59531C431),  // 用于计算哈希的常量
    SPH_C64(0x74F3E8B98774CD74),  // 用于计算哈希的常量
    SPH_C64(0xF615F109E3F6FFF6),  // 用于计算哈希的常量
    SPH_C64(0x464C8C430A460546),  // 用于计算哈希的常量
    SPH_C64(0xACA5452609AC8AAC),  // 用于计算哈希的常量
    SPH_C64(0x89B50F973C891E89),  // 用于计算哈希的常量
    SPH_C64(0x14B42844A0145014),  // 用于计算哈希的常量
    SPH_C64(0xE1BADF425BE1A3E1),  // 用于计算哈希的常量
    SPH_C64(0x16A62C4EB0165816),  // 用于计算哈希的常量
    SPH_C64(0x3AF774D2CD3AE83A),  // 用于计算哈希的常量
    SPH_C64(0x6906D2D06F69B969),  // 用于计算哈希的常量
    SPH_C64(0x0941122D48092409),  // 用于计算哈希的常量
    SPH_C64(0x70D7E0ADA770DD70),  // 用于计算哈希的常量
    SPH_C64(0xB66F7154D9B6E2B6),  // 用于计算哈希的常量
    SPH_C64(0xD01EBDB7CED067D0),  // 用于计算哈希的常量
    SPH_C64(0xEDD6C77E3BED93ED),  // 用于计算哈希的常量
    SPH_C64(0xCCE285DB2ECC17CC),  // 用于计算哈希的常量
    SPH_C64(0x426884572A421542),  // 用于计算哈希的常量
    SPH_C64(0x982C2DC2B4985A98),  // 用于计算哈希的常量
    SPH_C64(0xA4ED550E49A4AAA4),  // 用于计算哈希的常量
    SPH_C64(0x287550885D28A028),  // 用于计算哈希的常量
    SPH_C64(0x5C86B831DA5C6D5C),  // 用于计算哈希的常量
    # 使用宏SPH_C64对十六进制数进行64位常量的定义
...
static const sph_u64 old0_T0[256] = {
    ...
    ...
    SPH_C64(0x0307F303F401F4F4),  // 用十六进制表示的常量
    SPH_C64(0xEE9BADEE365A3636),  // 用十六进制表示的常量
    SPH_C64(0xB217C6B2D16ED1D1),  // 用十六进制表示的常量
    SPH_C64(0xDA147FDA6BBD6B6B),  // 用十六进制表示的常量
    SPH_C64(0x77C3D8771B2D1B1B),  // 用十六进制表示的常量
    SPH_C64(0xEC6A0FEC65AF6565),  // 用十六进制表示的常量
    SPH_C64(0xBCFA8FBC759F7575),  // 用十六进制表示的常量
    SPH_C64(0x5090805010301010),  // 用十六进制表示的常量
    SPH_C64(0x95449E95DA73DADA),  // 用十六进制表示的常量
    SPH_C64(0x703B727049DB4949),  // 用十六进制表示的常量
    SPH_C64(0xBE0B2DBE266A2626),  // 用十六进制表示的常量
    SPH_C64(0x3A629B3AF916F9F9),  // 用十六进制表示的常量
    SPH_C64(0xC0DD16C0CB40CBCB),  // 用十六进制表示的常量
    SPH_C64(0xE37117E366AA6666),  // 用十六进制表示的常量
    SPH_C64(0x5C8C6B5CE734E7E7),  // 用十六进制表示的常量
    SPH_C64(0x6803B968BAD3BABA),  // 用十六进制表示的常量
    SPH_C64(0x2CB7192CAEEFAEAE),  // 用十六进制表示的常量
    SPH_C64(0x0DEABA0D50F05050),  // 用十六进制表示的常量
    SPH_C64(0x07F8AA0752F65252),  // 用十六进制表示的常量
    SPH_C64(0x3D9A313DABE0ABAB),  // 用十六进制表示的常量
    SPH_C64(0x112D2811050F0505),  // 用十六进制表示的常量
    SPH_C64(0x1723D317F00DF0F0),  // 用十六进制表示的常量
    SPH_C64(0x396568390D170D0D),  // 用十六进制表示的常量
    SPH_C64(0xA2CCBFA273957373),  // 用十六进制表示的常量
    SPH_C64(0xD7FEC5D73B4D3B3B),  // 用十六进制表示的常量
    SPH_C64(0x14242014040C0404),  // 用十六进制表示的常量
    SPH_C64(0xA03D1DA020602020),  // 用十六进制表示的常量
    SPH_C64(0x215DA321FE1FFEFE),  // 用十六进制表示的常量
    SPH_C64(0x8E7BA68EDD7ADDDD),  // 用十六进制表示的常量
    SPH_C64(0x060EFB06F502F5F5),  // 用十六进制表示的常量
    SPH_C64(0x5E7DC95EB4C1B4B4),  // 用十六进制表示的常量
    SPH_C64(0x3E9DC23E5FE15F5F),  // 用十六进制表示的常量
    SPH_C64(0x225A50220A1E0A0A),  // 用十六进制表示的常量
    SPH_C64(0x5B74C15BB5C2B5B5),  // 用十六进制表示的常量
    SPH_C64(0xE78E4EE7C05DC0C0),  // 用十六进制表示的常量
    SPH_C64(0x1AC9691AA0FDA0A0),  // 用十六进制表示的常量
    SPH_C64(0xA8DEAFA871937171),  // 用十六进制表示的常量
    SPH_C64(0x0BE4410BA5F2A5A5),  // 用十六进制表示的常量
    SPH_C64(0x995875992D772D2D),  // 用十六进制表示的常量
    SPH_C64(0xFD4727FD60A06060),  // 用十六进制表示的常量
    SPH_C64(0xA7C5B7A772967272),  // 用十六进制表示的常量
    SPH_C64(0xE57FECE593A89393),  // 用十六进制表示的常量
    SPH_C64(0xDDECD5DD394B3939),  // 用十六进制表示的常量
    SPH_C64(0x2848402808180808),  // 用十六进制表示的常量
    SPH_C64(0xB5EF6CB583988383),  // 用十六进制表示的常量
    SPH_C64(0xA53415A521632121),  // 用十六进制表示的常量
    SPH_C64(0x3186DA315CE45C5C),  // 用十六进制表示的常量
    SPH_C64(0xA1CB4CA187948787),  // 用十六进制表示的常量
    SPH_C64(0x4F50E14FB1CEB1B1),  // 用十六进制表示的常量
    SPH_C64(0x47B35347E03DE0E0),  // 用十六进制表示的常量
    SPH_C64(0x0000000000000000),  // 用十六进制表示的常量
    SPH_C64(0xE89556E8C358C3C3),  // 用十六进制表示的常量
    SPH_C64(0x5A82905A12361212),  // 用十六进制表示的常量
    SPH_C64(0xEF6DFCEF91AE9191),  // 用十六进制表示的常量
    SPH_C64(0x98AE24988A838A8A),  // 用十六进制表示的常量
    SPH_C64(0x0A12100A02060202),  // 用十六进制表示的常量
    SPH_C64(0x6CFCE06C1C241C1C),  // 用十六进制表示的常量
    SPH_C64(0x59856359E637E6E6),  // 用十六进制表示的常量
    SPH_C64(0x4C57124C45CF4545),  // 用十六进制表示的常量
    SPH_C64(0xED9C5EEDC25BC2C2),  // 用十六进制表示的常量
    SPH_C64(0xF3AA6EF3C451C4C4),  // 用十六进制表示的常量
    SPH_C64(0x2E46BB2EFD1AFDFD),  // 用十六进制表示的常量
    SPH_C64(0x792E9179BFDCBFBF),  // 用十六进制表示的常量
    SPH_C64(0x495E1A4944CC4444),  // 用十六进制表示的常量
    # 定义一个常量数组，包含一系列64位的十六进制数值
    ...
    # 定义一系列的64位常量，用于后续计算
    # 定义一系列64位常数，使用SPH_C64函数将16进制数转换为64位整数
...
# 定义一个名为old0_T1的静态常量数组，包含256个sph_u64类型的元素
# 这个数组用于存储一些预定义的64位整数常量
...
static const sph_u64 old0_T2[256] = {
    ...
    SPH_C64(0x26DECD4ACDCDDEEB),  // 64位常量的十六进制表示
    SPH_C64(0xFEA9D667D6D6A928),  // 64位常量的十六进制表示
    SPH_C64(0x47C16CB46C6CC12B),  // 64位常量的十六进制表示
    SPH_C64(0xD151B7C4B7B75166),  // 64位常量的十六进制表示
    SPH_C64(0x933FF815F8F83F6B),  // 64位常量的十六进制表示
    SPH_C64(0x482D091B09092D41),  // 64位常量的十六进制表示
    SPH_C64(0xCB18F308F3F31838),  // 64位常量的十六进制表示
    SPH_C64(0x1FE667A96767E678),  // 64位常量的十六进制表示
    SPH_C64(0x490EA4F1A4A40EED),  // 64位常量的十六进制表示
    SPH_C64(0x0365EA23EAEA65E9),  // 64位常量的十六进制表示
    SPH_C64(0x337BEC29ECEC7BDF),  // 64位常量的十六进制表示
    SPH_C64(0xD954B6C7B6B6546F),  // 64位常量的十六进制表示
    SPH_C64(0xEEA3D461D4D4A33A),  // 64位常量的十六进制表示
    SPH_C64(0xDEBDD26BD2D2BD0C),  // 64位常量的十六进制表示
    SPH_C64(0xA044143C141444B4),  // 64位常量的十六进制表示
    SPH_C64(0xF0661E221E1E66EE),  // 64位常量的十六进制表示
    SPH_C64(0x5B42E13EE1E142BA),  // 64位常量的十六进制表示
    SPH_C64(0x3DB4246C2424B419),  // 64位常量的十六进制表示
    SPH_C64(0xDDD838483838D8E5),  // 64位常量的十六进制表示
    SPH_C64(0x7EF9C657C6C6F9B8),  // 64位常量的十六进制表示
    SPH_C64(0x9690DB70DBDB904D),  // 64位常量的十六进制表示
    SPH_C64(0x627A4BDD4B4B7A29),  // 64位常量的十六进制表示
    SPH_C64(0xF78F7A8E7A7A8F8D),  // 64位常量的十六进制表示
    SPH_C64(0xCDD23A4E3A3AD2F7),  // 64位常量的十六进制表示
    SPH_C64(0xBE81DE7FDEDE8160),  // 64位常量的十六进制表示
    SPH_C64(0xCA3B5EE25E5E3B94),  // 64位常量的十六进制表示
    SPH_C64(0xB684DF7CDFDF8469),  // 64位常量的十六进制表示
    SPH_C64(0xDCFB95A29595FB49),  // 64位常量的十六进制表示
    SPH_C64(0xB32BFC19FCFC2B4F),  // 64位常量的十六进制表示
    SPH_C64(0x3938AAE3AAAA3893),  // 64位常量的十六进制表示
    SPH_C64(0xF6ACD764D7D7AC21),  // 64位常量的十六进制表示
    SPH_C64(0x3ED1CE4FCECED1F0),  // 64位常量的十六进制表示
    SPH_C64(0x381B070907071B3F),  // 64位常量的十六进制表示
    SPH_C64(0x78330F110F0F3377),  // 64位常量的十六进制表示
    SPH_C64(0xF5C93D473D3DC9C8),  // 64位常量的十六进制表示
    SPH_C64(0xFA2558E8585825A2),  // 64位常量的十六进制表示
    SPH_C64(0xA4C89AB39A9AC83E),  // 64位常量的十六进制表示
    SPH_C64(0xB4C298B59898C22C),  // 64位常量的十六进制表示
    SPH_C64(0x94D69CB99C9CD608),  // 64位常量的十六进制表示
    SPH_C64(0xC31DF20BF2F21D31),  // 64位常量的十六进制表示
    SPH_C64(0x5101A7F4A7A701F6),  // 64位常量的十六进制表示
    SPH_C64(0x8855113311115599),  // 64位常量的十六进制表示
    SPH_C64(0xD79B7E827E7E9BA9),  // 64位常量的十六进制表示
    SPH_C64(0x2C9D8B808B8B9DA7),  // 64位常量的十六进制表示
    SPH_C64(0x225243C543435261),  // 64位常量的十六进制表示
    SPH_C64(0x180F030503030F1B),  // 64位常量的十六进制表示
    SPH_C64(0x434DE23BE2E24DA1),  // 64位常量的十六进制表示
    SPH_C64(0xAE8BDC79DCDC8B72),  // 64位常量的十六进制表示
    SPH_C64(0x7B56E532E5E5569E),  // 64位常量的十六进制表示
    SPH_C64(0xF940B2CBB2B2404B),  // 64位常量的十六进制表示
    SPH_C64(0x4A6B4ED24E4E6B04),  // 64位常量的十六进制表示
    SPH_C64(0x76FCC754C7C7FCB1),  // 64位常量的十六进制表示
    SPH_C64(0x4FC46DB76D6DC422),  // 64位常量的十六进制表示
    SPH_C64(0x1B6AE926E9E96AF2),  // 64位常量的十六进制表示
    SPH_C64(0x25BB27692727BB02),  // 64位常量的十六进制表示
    SPH_C64(0x3A5D40C040405D7A),  // 64位常量的十六进制表示
    SPH_C64(0x8E9FD875D8D89F56),  // 64位常量的十六进制表示
    SPH_C64(0xA5EB37593737EB92),  // 64位常量的十六进制表示
    SPH_C64(0xE4E092AB9292E076),  // 64位常量的十六进制表示
    SPH_C64(0x0C898F8C8F8F8983),  // 64位常量的十六进制表示
    SPH_C64(0x0805010301010509),  // 64位常量的十六进制表示
    SPH_C64(0xE8691D271D1D69F5),  // 64位常量的十六进制表示
    SPH_C64(0xA20253F5535302F1),  // 64位常量的十六进制表示
    SPH_C64(0xEDC63E423E3EC6D3),  // 64位常量的十六进制表示
    SPH_C64(0xF22059EB595920AB),  // 将十六进制数转换为64位整数
    SPH_C64(0x46E2C15EC1C1E287),
    SPH_C64(0x426E4FD14F4F6E0D),
    SPH_C64(0x8DFA32563232FABF),
    SPH_C64(0xB04E163A16164EA6),
    SPH_C64(0x8335FA13FAFA3579),
    SPH_C64(0x87B9749C7474B9F3),
    SPH_C64(0x8B30FB10FBFB3070),
    SPH_C64(0x3FF263A56363F25C),
    SPH_C64(0x8CD99FBC9F9FD913),
    SPH_C64(0xBDE4345C3434E489),
    SPH_C64(0xD0721A2E1A1A72CA),
    SPH_C64(0x4D822A7E2A2A8267),
    SPH_C64(0xEA2F5AEE5A5A2FB0),
    SPH_C64(0x1C838D8A8D8D8391),
    SPH_C64(0x06CAC946C9C9CACF),
    SPH_C64(0x36D4CF4CCFCFD4F9),
    SPH_C64(0xE309F607F6F60915),
    SPH_C64(0xF4EA90AD9090EA64),
    SPH_C64(0x5D88287828288875),
    SPH_C64(0x34928885888892BC),
    SPH_C64(0xACCD9BB09B9BCD37),
    SPH_C64(0x95F531533131F5A4),
    SPH_C64(0x70360E120E0E367E),
    SPH_C64(0x8173BDDABDBD733C),
    SPH_C64(0x6A7F4ADE4A4A7F20),
    SPH_C64(0x136FE825E8E86FFB),
    SPH_C64(0xC4F496A79696F452),
    SPH_C64(0x5904A6F7A6A604FF),
    SPH_C64(0x603C0C140C0C3C6C),
    SPH_C64(0x0ECFC845C8C8CFC6),
    SPH_C64(0xEF80798B79798096),
    SPH_C64(0x8976BCD9BCBC7635),
    SPH_C64(0x997CBEDFBEBE7C27),
    SPH_C64(0x2B74EF2CEFEF74C4),
    SPH_C64(0x57CB6EB26E6ECB39),
    SPH_C64(0x0A4346CA4646434C),
    SPH_C64(0xCCF197A49797F15B),
    SPH_C64(0xE22A5BED5B5B2AB9),
    SPH_C64(0x3B7EED2AEDED7ED6),
    SPH_C64(0xC87D192B19197DD1),
    SPH_C64(0x869AD976D9D99A5F),
    SPH_C64(0x0926ACE9ACAC26A5),
    SPH_C64(0xBCC799B69999C725),
    SPH_C64(0x2932A8E5A8A83281),
    SPH_C64(0x558D297B29298D7C),
    SPH_C64(0x07E964AC6464E963),
    SPH_C64(0xF8631F211F1F63E7),
    SPH_C64(0x0123ADEAADAD23AC),
    SPH_C64(0x921C55FF55551CC7),
    SPH_C64(0x985F133513135F8B),
    SPH_C64(0xB16DBBD0BBBB6D0A),
    SPH_C64(0xEB0CF704F7F70C1C),
    SPH_C64(0x5FCE6FB16F6FCE30),
    SPH_C64(0xA167B9D6B9B96718),
    SPH_C64(0x024647C947474645),
    SPH_C64(0x65932F712F2F934A),
    SPH_C64(0x2371EE2FEEEE71CD),
    SPH_C64(0xA962B8D5B8B86211),
    SPH_C64(0xFF8A7B8D7B7B8A84),
    SPH_C64(0x3C978986898997B5),
    SPH_C64(0x9DF030503030F0AD),
    SPH_C64(0xD6B8D368D3D3B805),
    SPH_C64(0xDF9E7F817F7F9EA0),
    # 使用SPH_C64函数对0x97B3769A7676B3E1进行处理
    SPH_C64(0x97B3769A7676B3E1), 
    # 使用SPH_C64函数对0x64B0829B8282B0E6进行处理
    SPH_C64(0x64B0829B8282B0E6)
# 定义一个静态的64位无符号整数数组，用于存储常量
...
# 定义一个静态的64位无符号整数数组，用于存储预定义的常量值
static const sph_u64 old0_T4[256] = {
    # 依次初始化256个元素，每个元素都是一个64位的常量值
    ...
    # 定义一系列的64位常量，用于后续计算
    SPH_C64(0xCD4ACDCDDEEB26DE),  // 计算 SHA-512 常量
    SPH_C64(0xD667D6D6A928FEA9),  // 计算 SHA-512 常量
    SPH_C64(0x6CB46C6CC12B47C1),  // 计算 SHA-512 常量
    SPH_C64(0xB7C4B7B75166D151),  // 计算 SHA-512 常量
    SPH_C64(0xF815F8F83F6B933F),  // 计算 SHA-512 常量
    SPH_C64(0x091B09092D41482D),  // 计算 SHA-512 常量
    SPH_C64(0xF308F3F31838CB18),  // 计算 SHA-512 常量
    SPH_C64(0x67A96767E6781FE6),  // 计算 SHA-512 常量
    SPH_C64(0xA4F1A4A40EED490E),  // 计算 SHA-512 常量
    SPH_C64(0xEA23EAEA65E90365),  // 计算 SHA-512 常量
    SPH_C64(0xEC29ECEC7BDF337B),  // 计算 SHA-512 常量
    SPH_C64(0xB6C7B6B6546FD954),  // 计算 SHA-512 常量
    SPH_C64(0xD461D4D4A33AEEA3),  // 计算 SHA-512 常量
    SPH_C64(0xD26BD2D2BD0CDEBD),  // 计算 SHA-512 常量
    SPH_C64(0x143C141444B4A044),  // 计算 SHA-512 常量
    SPH_C64(0x1E221E1E66EEF066),  // 计算 SHA-512 常量
    SPH_C64(0xE13EE1E142BA5B42),  // 计算 SHA-512 常量
    SPH_C64(0x246C2424B4193DB4),  // 计算 SHA-512 常量
    SPH_C64(0x38483838D8E5DDD8),  // 计算 SHA-512 常量
    SPH_C64(0xC657C6C6F9B87EF9),  // 计算 SHA-512 常量
    SPH_C64(0xDB70DBDB904D9690),  // 计算 SHA-512 常量
    SPH_C64(0x4BDD4B4B7A29627A),  // 计算 SHA-512 常量
    SPH_C64(0x7A8E7A7A8F8DF78F),  // 计算 SHA-512 常量
    SPH_C64(0x3A4E3A3AD2F7CDD2),  // 计算 SHA-512 常量
    SPH_C64(0xDE7FDEDE8160BE81),  // 计算 SHA-512 常量
    SPH_C64(0x5EE25E5E3B94CA3B),  // 计算 SHA-512 常量
    SPH_C64(0xDF7CDFDF8469B684),  // 计算 SHA-512 常量
    SPH_C64(0x95A29595FB49DCFB),  // 计算 SHA-512 常量
    SPH_C64(0xFC19FCFC2B4FB32B),  // 计算 SHA-512 常量
    SPH_C64(0xAAE3AAAA38933938),  // 计算 SHA-512 常量
    SPH_C64(0xD764D7D7AC21F6AC),  // 计算 SHA-512 常量
    SPH_C64(0xCE4FCECED1F03ED1),  // 计算 SHA-512 常量
    SPH_C64(0x070907071B3F381B),  // 计算 SHA-512 常量
    SPH_C64(0x0F110F0F33777833),  // 计算 SHA-512 常量
    SPH_C64(0x3D473D3DC9C8F5C9),  // 计算 SHA-512 常量
    SPH_C64(0x58E8585825A2FA25),  // 计算 SHA-512 常量
    SPH_C64(0x9AB39A9AC83EA4C8),  // 计算 SHA-512 常量
    SPH_C64(0x98B59898C22CB4C2),  // 计算 SHA-512 常量
    SPH_C64(0x9CB99C9CD60894D6),  // 计算 SHA-512 常量
    SPH_C64(0xF20BF2F21D31C31D),  // 计算 SHA-512 常量
    SPH_C64(0xA7F4A7A701F65101),  // 计算 SHA-512 常量
    SPH_C64(0x1133111155998855),  // 计算 SHA-512 常量
    SPH_C64(0x7E827E7E9BA9D79B),  // 计算 SHA-512 常量
    SPH_C64(0x8B808B8B9DA72C9D),  // 计算 SHA-512 常量
    SPH_C64(0x43C5434352612252),  // 计算 SHA-512 常量
    SPH_C64(0x030503030F1B180F),  // 计算 SHA-512 常量
    SPH_C64(0xE23BE2E24DA1434D),  // 计算 SHA-512 常量
    SPH_C64(0xDC79DCDC8B72AE8B),  // 计算 SHA-512 常量
    SPH_C64(0xE532E5E5569E7B56),  // 计算 SHA-512 常量
    SPH_C64(0xB2CBB2B2404BF940),  // 计算 SHA-512 常量
    SPH_C64(0x4ED24E4E6B044A6B),  // 计算 SHA-512 常量
    SPH_C64(0xC754C7C7FCB176FC),  // 计算 SHA-512 常量
    SPH_C64(0x6DB76D6DC4224FC4),  // 计算 SHA-512 常量
    SPH_C64(0xE926E9E96AF21B6A),  // 计算 SHA-512 常量
    SPH_C64(0x27692727BB0225BB),  // 计算 SHA-512 常量
    SPH_C64(0x40C040405D7A3A5D),  // 计算 SHA-512 常量
    SPH_C64(0xD875D8D89F568E9F),  // 计算 SHA-512 常量
    SPH_C64(0x37593737EB92A5EB),  // 计算 SHA-512 常量
    SPH_C64(0x92AB9292E076E4E0),  // 计算 SHA-512 常量
    SPH_C64(0x8F8C8F8F89830C89),  // 计算 SHA-512 常量
    SPH_C64(0x0103010105090805),  // 计算 SHA-512 常量
    SPH_C64(0x1D271D1D69F5E869),  // 计算 SHA-512 常量
    SPH_C64(0x53F5535302F1A202),  // 计算 SHA-512 常量
    SPH_C64(0x3E423E3EC6D3EDC6),  // 计算 SHA-512 常量
    SPH_C64(0x59EB595920ABF220),  // 用常量 0x59EB595920ABF220 初始化一个64位整数
    SPH_C64(0xC15EC1C1E28746E2),  // 用常量 0xC15EC1C1E28746E2 初始化一个64位整数
    SPH_C64(0x4FD14F4F6E0D426E),  // 用常量 0x4FD14F4F6E0D426E 初始化一个64位整数
    SPH_C64(0x32563232FABF8DFA),  // 用常量 0x32563232FABF8DFA 初始化一个64位整数
    SPH_C64(0x163A16164EA6B04E),  // 用常量 0x163A16164EA6B04E 初始化一个64位整数
    SPH_C64(0xFA13FAFA35798335),  // 用常量 0xFA13FAFA35798335 初始化一个64位整数
    SPH_C64(0x749C7474B9F387B9),  // 用常量 0x749C7474B9F387B9 初始化一个64位整数
    SPH_C64(0xFB10FBFB30708B30),  // 用常量 0xFB10FBFB30708B30 初始化一个64位整数
    SPH_C64(0x63A56363F25C3FF2),  // 用常量 0x63A56363F25C3FF2 初始化一个64位整数
    SPH_C64(0x9FBC9F9FD9138CD9),  // 用常量 0x9FBC9F9FD9138CD9 初始化一个64位整数
    SPH_C64(0x345C3434E489BDE4),  // 用常量 0x345C3434E489BDE4 初始化一个64位整数
    SPH_C64(0x1A2E1A1A72CAD072),  // 用常量 0x1A2E1A1A72CAD072 初始化一个64位整数
    SPH_C64(0x2A7E2A2A82674D82),  // 用常量 0x2A7E2A2A82674D82 初始化一个64位整数
    SPH_C64(0x5AEE5A5A2FB0EA2F),  // 用常量 0x5AEE5A5A2FB0EA2F 初始化一个64位整数
    SPH_C64(0x8D8A8D8D83911C83),  // 用常量 0x8D8A8D8D83911C83 初始化一个64位整数
    SPH_C64(0xC946C9C9CACF06CA),  // 用常量 0xC946C9C9CACF06CA 初始化一个64位整数
    SPH_C64(0xCF4CCFCFD4F936D4),  // 用常量 0xCF4CCFCFD4F936D4 初始化一个64位整数
    SPH_C64(0xF607F6F60915E309),  // 用常量 0xF607F6F60915E309 初始化一个64位整数
    SPH_C64(0x90AD9090EA64F4EA),  // 用常量 0x90AD9090EA64F4EA 初始化一个64位整数
    SPH_C64(0x2878282888755D88),  // 用常量 0x2878282888755D88 初始化一个64位整数
    SPH_C64(0x8885888892BC3492),  // 用常量 0x8885888892BC3492 初始化一个64位整数
    SPH_C64(0x9BB09B9BCD37ACCD),  // 用常量 0x9BB09B9BCD37ACCD 初始化一个64位整数
    SPH_C64(0x31533131F5A495F5),  // 用常量 0x31533131F5A495F5 初始化一个64位整数
    SPH_C64(0x0E120E0E367E7036),  // 用常量 0x0E120E0E367E7036 初始化一个64位整数
    SPH_C64(0xBDDABDBD733C8173),  // 用常量 0xBDDABDBD733C8173 初始化一个64位整数
    SPH_C64(0x4ADE4A4A7F206A7F),  // 用常量 0x4ADE4A4A7F206A7F 初始化一个64位整数
    SPH_C64(0xE825E8E86FFB136F),  // 用常量 0xE825E8E86FFB136F 初始化一个64位整数
    SPH_C64(0x96A79696F452C4F4),  // 用常量 0x96A79696F452C4F4 初始化一个64位整数
    SPH_C64(0xA6F7A6A604FF5904),  // 用常量 0xA6F7A6A604FF5904 初始化一个64位整数
    SPH_C64(0x0C140C0C3C6C603C),  // 用常量 0x0C140C0C3C6C603C 初始化一个64位整数
    SPH_C64(0xC845C8C8CFC60ECF),  // 用常量 0xC845C8C8CFC60ECF 初始化一个64位整数
    SPH_C64(0x798B79798096EF80),  // 用常量 0x798B79798096EF80 初始化一个64位整数
    SPH_C64(0xBCD9BCBC76358976),  // 用常量 0xBCD9BCBC76358976 初始化一个64位整数
    SPH_C64(0xBEDFBEBE7C27997C),  // 用常量 0xBEDFBEBE7C27997C 初始化一个64位整数
    SPH_C64(0xEF2CEFEF74C42B74),  // 用常量 0xEF2CEFEF74C42B74 初始化一个64位整数
    SPH_C64(0x6EB26E6ECB3957CB),  // 用常量 0x6EB26E6ECB3957CB 初始化一个64位整数
    SPH_C64(0x46CA4646434C0A43),  // 用常量 0x46CA4646434C0A43 初始化一个64位整数
    SPH_C64(0x97A49797F15BCCF1),  // 用常量 0x97A49797F15BCCF1 初始化一个64位整数
    SPH_C64(0x5BED5B5B2AB9E22A),  // 用常量 0x5BED5B5B2AB9E22A 初始化一个64位整数
    SPH_C64(0xED2AEDED7ED63B7E),  // 用常量 0xED2AEDED7ED63B7E 初始化一个64位整数
    SPH_C64(0x192B19197DD1C87D),  // 用常量 0x192B19197DD1C87D 初始化一个64位整数
    SPH_C64(0xD976D9D99A5F869A),  // 用常量 0xD976D9D99A5F869A 初始化一个64位整数
    SPH_C64(0xACE9ACAC26A50926),  // 用常量 0xACE9ACAC26A50926 初始化一个64位整数
    SPH_C64(0x99B69999C725BCC7),  // 用常量 0x99B69999C725BCC7 初始化一个64位整数
    SPH_C64(0xA8E5A8A832812932),  // 用常量 0xA8E5A8A832812932 初始化一个64位整数
    SPH_C64(0x297B29298D7C558D),  // 用常量 0x297B29298D7C558D 初始化一个64位整数
    SPH_C64(0x64AC6464E96307E9),  // 用常量 0x64AC6464E96307E9 初始化一个64位整数
    SPH_C64(0x1F211F1F63E7F863),  // 用常量 0x1F211F1F63E7F863 初始化一个64位整数
    SPH_C64(0xADEAADAD23AC0123),  // 用常量 0xADEAADAD23AC0123 初始化一个64位整数
    SPH_C64(0x55FF55551CC7921C),  // 用常量 0x55FF55551CC7921C 初始化一个64位整数
    SPH_C64(0x133513135F8B985F),  // 用常量 0x133513135F8B985F 初始化一个64位整数
    SPH_C64(0xBBD0BBBB6D0AB16D),  // 用常量 0xBBD0BBBB6D0AB16D 初始化一个64位整数
    SPH_C64(0xF704F7F70C1CEB0C),  // 用常量 0xF704F7F70C1CEB0C 初始化一个64位整数
    SPH_C64(0x6FB16F6FCE305FCE),  // 用常量 0x6FB16F6FCE305FCE 初始化一个64位整数
    SPH_C64(0xB9D6B9B96718A167),  // 用常量 0xB9D6B9B96718A167 初始化一个64位整数
    SPH_C64(0x47C9474746450246),  // 用常量 0x47C9474746450246 初始化一个64位整数
    SPH_C64(0x2F712F2F934A6593),  // 用常量 0x2F712F2F934A6593 初始化一个64位整数
    SPH_C64(0xEE2FEEEE71CD2371),  // 用常量 0xEE2FEEEE71CD2371 初始化一个64位整数
    SPH_C64(0xB8D5B8B86211A962),  // 用常量 0xB8D5B8B86211A962 初始化一个64位整数
    SPH_C64(0x7B8D7B7B8A84FF8A),  // 用常量 0x7B8D7B7B8A84FF8A 初始化一个64位整数
    SPH_C64(0x8986898997B53C97),  // 用常量 0x8986898997B53C97 初始化一个64位整数
    SPH_C64(0x30503030F0AD9DF0),  // 用常量 0x30503030F0AD9DF0 初始化一个64位整数
    SPH_C64(0xD368D3D3B805D6B8),  // 用常量 0xD368D3D3B805D6B8 初始化一个64位整数
    SPH_C64(0x7F817F7F9EA0DF9E),  // 用常量 0x7F817F7F9EA0DF9E 初始化一个64位整数
    # 使用SPH_C64函数对0x769A7676B3E197B3进行处理
    SPH_C64(0x769A7676B3E197B3), 
    # 使用SPH_C64函数对0x829B8282B0E664B0进行处理
    SPH_C64(0x829B8282B0E664B0)
# 定义一个静态的64位无符号整数数组，用于存储一些常量值
static const sph_u64 old0_T5[256] = {
    # 下面是一些64位无符号整数的十六进制表示，这些值将在后续的代码中使用
    ...
    # 调用 SPH_C64 函数，传入参数 0x9A7676B3E197B376，并将结果作为参数传入 SPH_C64 函数，参数为 0x9B8282B0E664B082
# 定义一个静态的64位无符号整数数组，用于存储一些常量值
static const sph_u64 old0_T6[256] = {
    # 下面是一些常量值的定义，每个常量值都是一个64位的无符号整数
    ...
    SPH_C64(0xCDCDDEEB26DECD4A),  // 将十六进制数转换为64位整数
    SPH_C64(0xD6D6A928FEA9D667),
    SPH_C64(0x6C6CC12B47C16CB4),
    SPH_C64(0xB7B75166D151B7C4),
    SPH_C64(0xF8F83F6B933FF815),
    SPH_C64(0x09092D41482D091B),
    SPH_C64(0xF3F31838CB18F308),
    SPH_C64(0x6767E6781FE667A9),
    SPH_C64(0xA4A40EED490EA4F1),
    SPH_C64(0xEAEA65E90365EA23),
    SPH_C64(0xECEC7BDF337BEC29),
    SPH_C64(0xB6B6546FD954B6C7),
    SPH_C64(0xD4D4A33AEEA3D461),
    SPH_C64(0xD2D2BD0CDEBDD26B),
    SPH_C64(0x141444B4A044143C),
    SPH_C64(0x1E1E66EEF0661E22),
    SPH_C64(0xE1E142BA5B42E13E),
    SPH_C64(0x2424B4193DB4246C),
    SPH_C64(0x3838D8E5DDD83848),
    SPH_C64(0xC6C6F9B87EF9C657),
    SPH_C64(0xDBDB904D9690DB70),
    SPH_C64(0x4B4B7A29627A4BDD),
    SPH_C64(0x7A7A8F8DF78F7A8E),
    SPH_C64(0x3A3AD2F7CDD23A4E),
    SPH_C64(0xDEDE8160BE81DE7F),
    SPH_C64(0x5E5E3B94CA3B5EE2),
    SPH_C64(0xDFDF8469B684DF7C),
    SPH_C64(0x9595FB49DCFB95A2),
    SPH_C64(0xFCFC2B4FB32BFC19),
    SPH_C64(0xAAAA38933938AAE3),
    SPH_C64(0xD7D7AC21F6ACD764),
    SPH_C64(0xCECED1F03ED1CE4F),
    SPH_C64(0x07071B3F381B0709),
    SPH_C64(0x0F0F337778330F11),
    SPH_C64(0x3D3DC9C8F5C93D47),
    SPH_C64(0x585825A2FA2558E8),
    SPH_C64(0x9A9AC83EA4C89AB3),
    SPH_C64(0x9898C22CB4C298B5),
    SPH_C64(0x9C9CD60894D69CB9),
    SPH_C64(0xF2F21D31C31DF20B),
    SPH_C64(0xA7A701F65101A7F4),
    SPH_C64(0x1111559988551133),
    SPH_C64(0x7E7E9BA9D79B7E82),
    SPH_C64(0x8B8B9DA72C9D8B80),
    SPH_C64(0x43435261225243C5),
    SPH_C64(0x03030F1B180F0305),
    SPH_C64(0xE2E24DA1434DE23B),
    SPH_C64(0xDCDC8B72AE8BDC79),
    SPH_C64(0xE5E5569E7B56E532),
    SPH_C64(0xB2B2404BF940B2CB),
    SPH_C64(0x4E4E6B044A6B4ED2),
    SPH_C64(0xC7C7FCB176FCC754),
    SPH_C64(0x6D6DC4224FC46DB7),
    SPH_C64(0xE9E96AF21B6AE926),
    SPH_C64(0x2727BB0225BB2769),
    SPH_C64(0x40405D7A3A5D40C0),
    SPH_C64(0xD8D89F568E9FD875),
    SPH_C64(0x3737EB92A5EB3759),
    SPH_C64(0x9292E076E4E092AB),
    SPH_C64(0x8F8F89830C898F8C),
    SPH_C64(0x0101050908050103),
    SPH_C64(0x1D1D69F5E8691D27),
    SPH_C64(0x535302F1A20253F5),
    SPH_C64(0x3E3EC6D3EDC63E42),
    # 定义一个常量数组，包含一系列64位的十六进制数值
    ...
    # 使用SPH_C64函数对0x7676B3E197B3769A进行处理
    SPH_C64(0x7676B3E197B3769A), 
    # 使用SPH_C64函数对0x8282B0E664B0829B进行处理
    SPH_C64(0x8282B0E664B0829B)
# 定义一个静态的64位无符号整数数组，用于存储一些常量值
static const sph_u64 old0_T7[256] = {
    ...
    SPH_C64(0xCDDEEB26DECD4ACD),  // 用于计算哈希的常量
    SPH_C64(0xD6A928FEA9D667D6),  // 用于计算哈希的常量
    SPH_C64(0x6CC12B47C16CB46C),  // 用于计算哈希的常量
    SPH_C64(0xB75166D151B7C4B7),  // 用于计算哈希的常量
    SPH_C64(0xF83F6B933FF815F8),  // 用于计算哈希的常量
    SPH_C64(0x092D41482D091B09),  // 用于计算哈希的常量
    SPH_C64(0xF31838CB18F308F3),  // 用于计算哈希的常量
    SPH_C64(0x67E6781FE667A967),  // 用于计算哈希的常量
    SPH_C64(0xA40EED490EA4F1A4),  // 用于计算哈希的常量
    SPH_C64(0xEA65E90365EA23EA),  // 用于计算哈希的常量
    SPH_C64(0xEC7BDF337BEC29EC),  // 用于计算哈希的常量
    SPH_C64(0xB6546FD954B6C7B6),  // 用于计算哈希的常量
    SPH_C64(0xD4A33AEEA3D461D4),  // 用于计算哈希的常量
    SPH_C64(0xD2BD0CDEBDD26BD2),  // 用于计算哈希的常量
    SPH_C64(0x1444B4A044143C14),  // 用于计算哈希的常量
    SPH_C64(0x1E66EEF0661E221E),  // 用于计算哈希的常量
    SPH_C64(0xE142BA5B42E13EE1),  // 用于计算哈希的常量
    SPH_C64(0x24B4193DB4246C24),  // 用于计算哈希的常量
    SPH_C64(0x38D8E5DDD8384838),  // 用于计算哈希的常量
    SPH_C64(0xC6F9B87EF9C657C6),  // 用于计算哈希的常量
    SPH_C64(0xDB904D9690DB70DB),  // 用于计算哈希的常量
    SPH_C64(0x4B7A29627A4BDD4B),  // 用于计算哈希的常量
    SPH_C64(0x7A8F8DF78F7A8E7A),  // 用于计算哈希的常量
    SPH_C64(0x3AD2F7CDD23A4E3A),  // 用于计算哈希的常量
    SPH_C64(0xDE8160BE81DE7FDE),  // 用于计算哈希的常量
    SPH_C64(0x5E3B94CA3B5EE25E),  // 用于计算哈希的常量
    SPH_C64(0xDF8469B684DF7CDF),  // 用于计算哈希的常量
    SPH_C64(0x95FB49DCFB95A295),  // 用于计算哈希的常量
    SPH_C64(0xFC2B4FB32BFC19FC),  // 用于计算哈希的常量
    SPH_C64(0xAA38933938AAE3AA),  // 用于计算哈希的常量
    SPH_C64(0xD7AC21F6ACD764D7),  // 用于计算哈希的常量
    SPH_C64(0xCED1F03ED1CE4FCE),  // 用于计算哈希的常量
    SPH_C64(0x071B3F381B070907),  // 用于计算哈希的常量
    SPH_C64(0x0F337778330F110F),  // 用于计算哈希的常量
    SPH_C64(0x3DC9C8F5C93D473D),  // 用于计算哈希的常量
    SPH_C64(0x5825A2FA2558E858),  // 用于计算哈希的常量
    SPH_C64(0x9AC83EA4C89AB39A),  // 用于计算哈希的常量
    SPH_C64(0x98C22CB4C298B598),  // 用于计算哈希的常量
    SPH_C64(0x9CD60894D69CB99C),  // 用于计算哈希的常量
    SPH_C64(0xF21D31C31DF20BF2),  // 用于计算哈希的常量
    SPH_C64(0xA701F65101A7F4A7),  // 用于计算哈希的常量
    SPH_C64(0x1155998855113311),  // 用于计算哈希的常量
    SPH_C64(0x7E9BA9D79B7E827E),  // 用于计算哈希的常量
    SPH_C64(0x8B9DA72C9D8B808B),  // 用于计算哈希的常量
    SPH_C64(0x435261225243C543),  // 用于计算哈希的常量
    SPH_C64(0x030F1B180F030503),  // 用于计算哈希的常量
    SPH_C64(0xE24DA1434DE23BE2),  // 用于计算哈希的常量
    SPH_C64(0xDC8B72AE8BDC79DC),  // 用于计算哈希的常量
    SPH_C64(0xE5569E7B56E532E5),  // 用于计算哈希的常量
    SPH_C64(0xB2404BF940B2CBB2),  // 用于计算哈希的常量
    SPH_C64(0x4E6B044A6B4ED24E),  // 用于计算哈希的常量
    SPH_C64(0xC7FCB176FCC754C7),  // 用于计算哈希的常量
    SPH_C64(0x6DC4224FC46DB76D),  // 用于计算哈希的常量
    SPH_C64(0xE96AF21B6AE926E9),  // 用于计算哈希的常量
    SPH_C64(0x27BB0225BB276927),  // 用于计算哈希的常量
    SPH_C64(0x405D7A3A5D40C040),  // 用于计算哈希的常量
    SPH_C64(0xD89F568E9FD875D8),  // 用于计算哈希的常量
    SPH_C64(0x37EB92A5EB375937),  // 用于计算哈希的常量
    SPH_C64(0x92E076E4E092AB92),  // 用于计算哈希的常量
    SPH_C64(0x8F89830C898F8C8F),  // 用于计算哈希的常量
    SPH_C64(0x0105090805010301),  // 用于计算哈希的常量
    SPH_C64(0x1D69F5E8691D271D),  // 用于计算哈希的常量
    SPH_C64(0x5302F1A20253F553),  // 用于计算哈希的常量
    SPH_C64(0x3EC6D3EDC63E423E),  // 用于计算哈希的常量
    SPH_C64(0x5920ABF22059EB59), // 用指定的常量值创建 64 位整数
    SPH_C64(0xC1E28746E2C15EC1), // 用指定的常量值创建 64 位整数
    SPH_C64(0x4F6E0D426E4FD14F), // 用指定的常量值创建 64 位整数
    SPH_C64(0x32FABF8DFA325632), // 用指定的常量值创建 64 位整数
    SPH_C64(0x164EA6B04E163A16), // 用指定的常量值创建 64 位整数
    SPH_C64(0xFA35798335FA13FA), // 用指定的常量值创建 64 位整数
    SPH_C64(0x74B9F387B9749C74), // 用指定的常量值创建 64 位整数
    SPH_C64(0xFB30708B30FB10FB), // 用指定的常量值创建 64 位整数
    SPH_C64(0x63F25C3FF263A563), // 用指定的常量值创建 64 位整数
    SPH_C64(0x9FD9138CD99FBC9F), // 用指定的常量值创建 64 位整数
    SPH_C64(0x34E489BDE4345C34), // 用指定的常量值创建 64 位整数
    SPH_C64(0x1A72CAD0721A2E1A), // 用指定的常量值创建 64 位整数
    SPH_C64(0x2A82674D822A7E2A), // 用指定的常量值创建 64 位整数
    SPH_C64(0x5A2FB0EA2F5AEE5A), // 用指定的常量值创建 64 位整数
    SPH_C64(0x8D83911C838D8A8D), // 用指定的常量值创建 64 位整数
    SPH_C64(0xC9CACF06CAC946C9), // 用指定的常量值创建 64 位整数
    SPH_C64(0xCFD4F936D4CF4CCF), // 用指定的常量值创建 64 位整数
    SPH_C64(0xF60915E309F607F6), // 用指定的常量值创建 64 位整数
    SPH_C64(0x90EA64F4EA90AD90), // 用指定的常量值创建 64 位整数
    SPH_C64(0x2888755D88287828), // 用指定的常量值创建 64 位整数
    SPH_C64(0x8892BC3492888588), // 用指定的常量值创建 64 位整数
    SPH_C64(0x9BCD37ACCD9BB09B), // 用指定的常量值创建 64 位整数
    SPH_C64(0x31F5A495F5315331), // 用指定的常量值创建 64 位整数
    SPH_C64(0x0E367E70360E120E), // 用指定的常量值创建 64 位整数
    SPH_C64(0xBD733C8173BDDABD), // 用指定的常量值创建 64 位整数
    SPH_C64(0x4A7F206A7F4ADE4A), // 用指定的常量值创建 64 位整数
    SPH_C64(0xE86FFB136FE825E8), // 用指定的常量值创建 64 位整数
    SPH_C64(0x96F452C4F496A796), // 用指定的常量值创建 64 位整数
    SPH_C64(0xA604FF5904A6F7A6), // 用指定的常量值创建 64 位整数
    SPH_C64(0x0C3C6C603C0C140C), // 用指定的常量值创建 64 位整数
    SPH_C64(0xC8CFC60ECFC845C8), // 用指定的常量值创建 64 位整数
    SPH_C64(0x798096EF80798B79), // 用指定的常量值创建 64 位整数
    SPH_C64(0xBC76358976BCD9BC), // 用指定的常量值创建 64 位整数
    SPH_C64(0xBE7C27997CBEDFBE), // 用指定的常量值创建 64 位整数
    SPH_C64(0xEF74C42B74EF2CEF), // 用指定的常量值创建 64 位整数
    SPH_C64(0x6ECB3957CB6EB26E), // 用指定的常量值创建 64 位整数
    SPH_C64(0x46434C0A4346CA46), // 用指定的常量值创建 64 位整数
    SPH_C64(0x97F15BCCF197A497), // 用指定的常量值创建 64 位整数
    SPH_C64(0x5B2AB9E22A5BED5B), // 用指定的常量值创建 64 位整数
    SPH_C64(0xED7ED63B7EED2AED), // 用指定的常量值创建 64 位整数
    SPH_C64(0x197DD1C87D192B19), // 用指定的常量值创建 64 位整数
    SPH_C64(0xD99A5F869AD976D9), // 用指定的常量值创建 64 位整数
    SPH_C64(0xAC26A50926ACE9AC), // 用指定的常量值创建 64 位整数
    SPH_C64(0x99C725BCC799B699), // 用指定的常量值创建 64 位整数
    SPH_C64(0xA832812932A8E5A8), // 用指定的常量值创建 64 位整数
    SPH_C64(0x298D7C558D297B29), // 用指定的常量值创建 64 位整数
    SPH_C64(0x64E96307E964AC64), // 用指定的常量值创建 64 位整数
    SPH_C64(0x1F63E7F8631F211F), // 用指定的常量值创建 64 位整数
    SPH_C64(0xAD23AC0123ADEAAD), // 用指定的常量值创建 64 位整数
    SPH_C64(0x551CC7921C55FF55), // 用指定的常量值创建 64 位整数
    SPH_C64(0x135F8B985F133513), // 用指定的常量值创建 64 位整数
    SPH_C64(0xBB6D0AB16DBBD0BB), // 用指定的常量值创建 64 位整数
    SPH_C64(0xF70C1CEB0CF704F7), // 用指定的常量值创建 64 位整数
    SPH_C64(0x6FCE305FCE6FB16F), // 用指定的常量值创建 64 位整数
    SPH_C64(0xB96718A167B9D6B9), // 用指定的常量值创建 64 位整数
    SPH_C64(0x474645024647C947), // 用指定的常量值创建 64 位整数
    SPH_C64(0x2F934A65932F712F), // 用指定的常量值创建 64 位整数
    SPH_C64(0xEE71CD2371EE2FEE), // 用指定的常量值创建 64 位整数
    SPH_C64(0xB86211A962B8D5B8), // 用指定的常量值创建 64 位整数
    SPH_C64(0x7B8A84FF8A7B8D7B), // 用指定的常量值创建 64 位整数
    SPH_C64(0x8997B53C97898689), // 用指定的常量值创建 64 位整数
    SPH_C64(0x30F0AD9DF0305030), // 用指定的常量值创建 64 位整数
    SPH_C64(0xD3B805D6B8D368D3), // 用指定的常量值创建 64 位整数
    SPH_C64(0x7F9EA0DF9E7F817F), // 用指定的常量值创建 64 位整数
    # 使用SPH_C64函数对0x76B3E197B3769A76进行处理
    SPH_C64(0x76B3E197B3769A76), 
    # 使用SPH_C64函数对0x82B0E664B0829B82进行处理
    SPH_C64(0x82B0E664B0829B82)
};
#endif

# 定义旧版本 WHIRLPOOL-1 的常量
...
static const sph_u64 old1_T0[256] = {
    ...
    # 定义一系列的64位常量，用于后续计算
    SPH_C64(0xDDECD5DD394B3939),  // 用常量生成64位整数
    SPH_C64(0x61165A614CD44C4C),  // 用常量生成64位整数
    SPH_C64(0x3B94CA3B5EE25E5E),  // 用常量生成64位整数
    SPH_C64(0x859FE78578887878),  // 用常量生成64位整数
    SPH_C64(0xD8E5DDD838483838),  // 用常量生成64位整数
    SPH_C64(0x869814868C898C8C),  // 用常量生成64位整数
    SPH_C64(0xB217C6B2D16ED1D1),  // 用常量生成64位整数
    SPH_C64(0x0BE4410BA5F2A5A5),  // 用常量生成64位整数
    SPH_C64(0x4DA1434DE23BE2E2),  // 用常量生成64位整数
    SPH_C64(0xF84E2FF861A36161),  // 用常量生成64位整数
    SPH_C64(0x4542F145B3C8B3B3),  // 用常量生成64位整数
    SPH_C64(0xA53415A521632121),  // 用常量生成64位整数
    SPH_C64(0xD60894D69CB99C9C),  // 用常量生成64位整数
    SPH_C64(0x66EEF0661E221E1E),  // 用常量生成64位整数
    SPH_C64(0x5261225243C54343),  // 用常量生成64位整数
    SPH_C64(0xFCB176FCC754C7C7),  // 用常量生成64位整数
    SPH_C64(0x2B4FB32BFC19FCFC),  // 用常量生成64位整数
    SPH_C64(0x14242014040C0404),  // 用常量生成64位整数
    SPH_C64(0x08E3B20851F35151),  // 用常量生成64位整数
    SPH_C64(0xC725BCC799B69999),  // 用常量生成64位整数
    SPH_C64(0xC4224FC46DB76D6D),  // 用常量生成64位整数
    SPH_C64(0x396568390D170D0D),  // 用常量生成64位整数
    SPH_C64(0x35798335FA13FAFA),  // 用常量生成64位整数
    SPH_C64(0x8469B684DF7CDFDF),  // 用常量生成64位整数
    SPH_C64(0x9BA9D79B7E827E7E),  // 用常量生成64位整数
    SPH_C64(0xB4193DB4246C2424),  // 用常量生成64位整数
    SPH_C64(0xD7FEC5D73B4D3B3B),  // 用常量生成64位整数
    SPH_C64(0x3D9A313DABE0ABAB),  // 用常量生成64位整数
    SPH_C64(0xD1F03ED1CE4FCECE),  // 用常量生成64位整数
    SPH_C64(0x5599885511331111),  // 用常量生成64位整数
    SPH_C64(0x89830C898F8C8F8F),  // 用常量生成64位整数
    SPH_C64(0x6B044A6B4ED24E4E),  // 用常量生成64位整数
    SPH_C64(0x5166D151B7C4B7B7),  // 用常量生成64位整数
    SPH_C64(0x60E00B60EB20EBEB),  // 用常量生成64位整数
    SPH_C64(0xCCC1FDCC3C443C3C),  // 用常量生成64位整数
    SPH_C64(0xBFFD7CBF819E8181),  // 用常量生成64位整数
    SPH_C64(0xFE40D4FE94A19494),  // 用常量生成64位整数
    SPH_C64(0x0C1CEB0CF704F7F7),  // 用常量生成64位整数
    SPH_C64(0x6718A167B9D6B9B9),  // 用常量生成64位整数
    SPH_C64(0x5F8B985F13351313),  // 用常量生成64位整数
    SPH_C64(0x9C517D9C2C742C2C),  // 用常量生成64位整数
    SPH_C64(0xB805D6B8D368D3D3),  // 用常量生成64位整数
    SPH_C64(0x5C8C6B5CE734E7E7),  // 用常量生成64位整数
    SPH_C64(0xCB3957CB6EB26E6E),  // 用常量生成64位整数
    SPH_C64(0xF3AA6EF3C451C4C4),  // 用常量生成64位整数
    SPH_C64(0x0F1B180F03050303),  // 用常量生成64位整数
    SPH_C64(0x13DC8A1356FA5656),  // 用常量生成64位整数
    SPH_C64(0x495E1A4944CC4444),  // 用常量生成64位整数
    SPH_C64(0x9EA0DF9E7F817F7F),  // 用常量生成64位整数
    SPH_C64(0x37882137A9E6A9A9),  // 用常量生成64位整数
    SPH_C64(0x82674D822A7E2A2A),  // 用常量生成64位整数
    SPH_C64(0x6D0AB16DBBD0BBBB),  // 用常量生成64位整数
    SPH_C64(0xE28746E2C15EC1C1),  // 用常量生成64位整数
    SPH_C64(0x02F1A20253F55353),  // 用常量生成64位整数
    SPH_C64(0x8B72AE8BDC79DCDC),  // 用常量生成64位整数
    SPH_C64(0x275358270B1D0B0B),  // 用常量生成64位整数
    SPH_C64(0xD3019CD39DBA9D9D),  // 用常量生成64位整数
    SPH_C64(0xC12B47C16CB46C6C),  // 用常量生成64位整数
    SPH_C64(0xF5A495F531533131),  // 用常量生成64位整数
    SPH_C64(0xB9F387B9749C7474),  // 用常量生成64位整数
    SPH_C64(0x0915E309F607F6F6),  // 用常量生成64位整数
    SPH_C64(0x434C0A4346CA4646),  // 用常量生成64位整数
    SPH_C64(0x26A50926ACE9ACAC),  // 用常量生成64位整数
    SPH_C64(0x97B53C9789868989),  // 用常量生成64位整数
    # 定义一系列64位的常量，使用SPH_C64函数将16进制数转换为64位整数
...
# 如果不是小内存占用的 Whirlpool 算法，则定义一个静态的常量数组 old1_T1
#if !SPH_SMALL_FOOTPRINT_WHIRLPOOL
...
注释：定义了一个静态的常量数组 old1_T1，用于 Whirlpool 算法的计算。数组中的每个元素都是一个 64 位的无符号整数。
    SPH_C64(0x49DCFB95A29595FB), // 用常量 0x49DCFB95A29595FB 创建 64 位整数
    SPH_C64(0x568E9FD875D8D89F), // 用常量 0x568E9FD875D8D89F 创建 64 位整数
    SPH_C64(0x708B30FB10FBFB30), // 用常量 0x708B30FB10FBFB30 创建 64 位整数
    SPH_C64(0xCD2371EE2FEEEE71), // 用常量 0xCD2371EE2FEEEE71 创建 64 位整数
    SPH_C64(0xBBC7917C847C7C91), // 用常量 0xBBC7917C847C7C91 创建 64 位整数
    SPH_C64(0x7117E366AA6666E3), // 用常量 0x7117E366AA6666E3 创建 64 位整数
    SPH_C64(0x7BA68EDD7ADDDD8E), // 用常量 0x7BA68EDD7ADDDD8E 创建 64 位整数
    SPH_C64(0xAFB84B173917174B), // 用常量 0xAFB84B173917174B 创建 64 位整数
    SPH_C64(0x45024647C9474746), // 用常量 0x45024647C9474746 创建 64 位整数
    SPH_C64(0x1A84DC9EBF9E9EDC), // 用常量 0x1A84DC9EBF9E9EDC 创建 64 位整数
    SPH_C64(0xD41EC5CA43CACAC5), // 用常量 0xD41EC5CA43CACAC5 创建 64 位整数
    SPH_C64(0x5875992D772D2D99), // 用常量 0x5875992D772D2D99 创建 64 位整数
    SPH_C64(0x2E9179BFDCBFBF79), // 用常量 0x2E9179BFDCBFBF79 创建 64 位整数
    SPH_C64(0x3F381B070907071B), // 用常量 0x3F381B070907071B 创建 64 位整数
    SPH_C64(0xAC0123ADEAADAD23), // 用常量 0xAC0123ADEAADAD23 创建 64 位整数
    SPH_C64(0xB0EA2F5AEE5A5A2F), // 用常量 0xB0EA2F5AEE5A5A2F 创建 64 位整数
    SPH_C64(0xEF6CB583988383B5), // 用常量 0xEF6CB583988383B5 创建 64 位整数
    SPH_C64(0xB685FF33553333FF), // 用常量 0xB685FF33553333FF 创建 64 位整数
    SPH_C64(0x5C3FF263A56363F2), // 用常量 0x5C3FF263A56363F2 创建 64 位整数
    SPH_C64(0x12100A020602020A), // 用常量 0x12100A020602020A 创建 64 位整数
    SPH_C64(0x933938AAE3AAAA38), // 用常量 0x933938AAE3AAAA38 创建 64 位整数
    SPH_C64(0xDEAFA871937171A8), // 用常量 0xDEAFA871937171A8 创建 64 位整数
    SPH_C64(0xC60ECFC845C8C8CF), // 用常量 0xC60ECFC845C8C8CF 创建 64 位整数
    SPH_C64(0xD1C87D192B19197D), // 用常量 0xD1C87D192B19197D 创建 64 位整数
    SPH_C64(0x3B727049DB494970), // 用常量 0x3B727049DB494970 创建 64 位整数
    SPH_C64(0x5F869AD976D9D99A), // 用常量 0x5F869AD976D9D99A 创建 64 位整数
    SPH_C64(0x31C31DF20BF2F21D), // 用常量 0x31C31DF20BF2F21D 创建 64 位整数
    SPH_C64(0xA84B48E338E3E348), // 用常量 0xA84B48E338E3E348 创建 64 位整数
    SPH_C64(0xB9E22A5BED5B5B2A), // 用常量 0xB9E22A5BED5B5B2A 创建 64 位整数
    SPH_C64(0xBC34928885888892), // 用常量 0xBC34928885888892 创建 64 位整数
    SPH_C64(0x3EA4C89AB39A9AC8), // 用常量 0x3EA4C89AB39A9AC8 创建 64 位整数
    SPH_C64(0x0B2DBE266A2626BE), // 用常量 0x0B2DBE266A2626BE 创建 64 位整数
    SPH_C64(0xBF8DFA32563232FA), // 用常量 0xBF8DFA32563232FA 创建 64 位整数
    SPH_C64(0x59E94AB0CDB0B04A), // 用常量 0x59E94AB0CDB0B04A 创建 64 位整数
    SPH_C64(0xF21B6AE926E9E96A), // 用常量 0xF21B6AE926E9E96A 创建 64 位整数
    SPH_C64(0x7778330F110F0F33), // 用常量 0x7778330F110F0F33 创建 64 位整数
    SPH_C64(0x33E6A6D562D5D5A6), // 用常量 0x33E6A6D562D5D5A6 创建 64 位整数
    SPH_C64(0xF474BA809D8080BA), // 用常量 0xF474BA809D8080BA 创建 64 位整数
    SPH_C64(0x27997CBEDFBEBE7C), // 用常量 0x27997CBEDFBEBE7C 创建 64 位整数
    SPH_C64(0xEB26DECD4ACDCDDE), // 用常量 0xEB26DECD4ACDCDDE 创建 64 位整数
    SPH_C64(0x89BDE4345C3434E4), // 用常量 0x89BDE4345C3434E4 创建 64 位整数
    SPH_C64(0x327A7548D8484875), // 用常量 0x327A7548D8484875 创建 64 位整数
    SPH_C64(0x54AB24FF1CFFFF24), // 用常量 0x54AB24FF1CFFFF24 创建 64 位整数
    SPH_C64(0x8DF78F7A8E7A7A8F), // 用常量 0x8DF78F7A8E7A7A8F 创建 64 位整数
    SPH_C64(0x64F4EA90AD9090EA), // 用常量 0x64F4EA90AD9090EA 创建 64 位整数
    SPH_C64(0x9DC23E5FE15F5F3E), // 用常量 0x9DC23E5FE15F5F3E 创建 64 位整数
    SPH_C64(0x3D1DA020602020A0), // 用常量 0x3D1DA020602020A0 创建 64 位整数
    SPH_C64(0x0F67D568B86868D5), // 用常量 0x0F67D568B86868D5 创建 64 位整数
    SPH_C64(0xCAD0721A2E1A1A72), // 用常量 0xCAD0721A2E1A1A72 创建 64 位整数
    SPH_C64(0xB7192CAEEFAEAE2C), // 用常量 0xB7192CAEEFAEAE2C 创建 64 位整数
    SPH_C64(0x7DC95EB4C1B4B45E), // 用常量 0x7DC95EB4C1B4B45E 创建 64 位整数
    SPH_C64(0xCE9A1954FC545419), // 用常量 0xCE9A1954FC545419 创建 64 位整数
    SPH_C64(0x7FECE593A89393E5), // 用常量 0x7FECE593A89393E5 创建 64 位整数
    SPH_C64(0x2F0DAA22662222AA), // 用常量 0x2F0DAA22662222AA 创建 64 位整数
    SPH_C64(0x6307E964AC6464E9), // 用常量 0x6307E964AC6464E9 创建 64 位整数
    SPH_C64(0x2ADB12F10EF1F112), // 用常量 0x2ADB12F10EF1F112 创建 64 位整数
    SPH_C64(0xCCBFA273957373A2), // 用常量 0xCCBFA273957373A2 创建 64 位整数
    SPH_C64(0x82905A123612125A), // 用常量 0x82905A123612125A 创建 64 位整数
    SPH_C64(0x7A3A5D40C040405D), // 用常量 0x7A3A5D40C040405D 创建 64 位整数
    SPH_C64(0x4840280818080828), // 用常量 0x4840280818080828 创建 64 位整数
    SPH_C64(0x9556E8C358C3C3E8), // 用常量 0x9556E8C358C3C3E8 创建 64 位整数
    SPH_C64(0xDF337BEC29ECEC7B), // 用常量 0xDF337BEC29ECEC7B 创建 64 位整数
    SPH_C64(0x4D9690DB70DBDB90), // 用常量 0x4D9690DB70DBDB90 创建 64 位整数
    SPH_C64(0xC0611FA1FEA1A11F), // 用常量 0xC0611FA1FEA1A11F 创建 64 位整数
    SPH_C64(0x911C838D8A8D8D83),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xC8F5C93D473D3DC9),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x5BCCF197A49797F1),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x0000000000000000),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xF936D4CF4CCFCFD4),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x6E45872B7D2B2B87),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xE197B3769A7676B3),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xE664B0829B8282B0),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x28FEA9D667D6D6A9),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xC3D8771B2D1B1B77),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x74C15BB5C2B5B55B),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xBE1129AFECAFAF29),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x1D77DF6ABE6A6ADF),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xEABA0D50F050500D),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x57124C45CF45454C),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x38CB18F308F3F318),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xAD9DF030503030F0),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xC42B74EF2CEFEF74),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xDAE5C33F413F3FC3),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xC7921C55FF55551C),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xDB7910A2FBA2A210),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xE90365EA23EAEA65),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x6A0FEC65AF6565EC),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x03B968BAD3BABA68),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x4A65932F712F2F93),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x8E4EE7C05DC0C0E7),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x60BE81DE7FDEDE81),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xFCE06C1C241C1C6C),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x46BB2EFD1AFDFD2E),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x1F52644DD74D4D64),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x76E4E092AB9292E0),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xFA8FBC759F7575BC),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x36301E060A06061E),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xAE24988A838A8A98),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x4BF940B2CBB2B240),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x856359E637E6E659),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x7E70360E120E0E36),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xE7F8631F211F1F63),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x5537F762A66262F7),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x3AEEA3D461D4D4A3),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x812932A8E5A8A832),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x52C4F496A79696F4),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x629B3AF916F9F93A),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xA366F6C552C5C5F6),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x1035B1256F2525B1),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xABF22059EB595920),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xD054AE84918484AE),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xC5B7A772967272A7),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xECD5DD394B3939DD),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x165A614CD44C4C61),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x94CA3B5EE25E5E3B),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x9FE7857888787885),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xE5DDD838483838D8),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x9814868C898C8C86),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x17C6B2D16ED1D1B2),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xE4410BA5F2A5A50B),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xA1434DE23BE2E24D),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x4E2FF861A36161F8),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x42F145B3C8B3B345),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x3415A521632121A5),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x0894D69CB99C9CD6),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xEEF0661E221E1E66),  // 以 64 位十六进制数表示的常量
    SPH_C64(0x61225243C5434352),  // 以 64 位十六进制数表示的常量
    SPH_C64(0xB176FCC754C7C7FC),  // 以 64 位十六进制数表示的常量
    # 定义一系列64位常量，并使用SPH_C64函数进行初始化
...
static const sph_u64 old1_T2[256] = {
    ...
    # 调用SPH_C64函数两次，分别传入0x933FF815F8F83F6B和0x44A486978686A4C2作为参数
    SPH_C64(0x933FF815F8F83F6B), SPH_C64(0x44A486978686A4C2)
# 定义一个静态的64位无符号整数数组，用于存储一些预定义的常量值
static const sph_u64 old1_T3[256] = {
    # 下面是一些预定义的常量值，每个值都是一个64位的无符号整数
    ...
    SPH_C64(0x838D8A8D8D83911C), // 以 64 位十六进制数表示的常量
    SPH_C64(0xC93D473D3DC9C8F5), // 以 64 位十六进制数表示的常量
    SPH_C64(0xF197A49797F15BCC), // 以 64 位十六进制数表示的常量
    SPH_C64(0x0000000000000000), // 以 64 位十六进制数表示的常量
    SPH_C64(0xD4CF4CCFCFD4F936), // 以 64 位十六进制数表示的常量
    SPH_C64(0x872B7D2B2B876E45), // 以 64 位十六进制数表示的常量
    SPH_C64(0xB3769A7676B3E197), // 以 64 位十六进制数表示的常量
    SPH_C64(0xB0829B8282B0E664), // 以 64 位十六进制数表示的常量
    SPH_C64(0xA9D667D6D6A928FE), // 以 64 位十六进制数表示的常量
    SPH_C64(0x771B2D1B1B77C3D8), // 以 64 位十六进制数表示的常量
    SPH_C64(0x5BB5C2B5B55B74C1), // 以 64 位十六进制数表示的常量
    SPH_C64(0x29AFECAFAF29BE11), // 以 64 位十六进制数表示的常量
    SPH_C64(0xDF6ABE6A6ADF1D77), // 以 64 位十六进制数表示的常量
    SPH_C64(0x0D50F050500DEABA), // 以 64 位十六进制数表示的常量
    SPH_C64(0x4C45CF45454C5712), // 以 64 位十六进制数表示的常量
    SPH_C64(0x18F308F3F31838CB), // 以 64 位十六进制数表示的常量
    SPH_C64(0xF030503030F0AD9D), // 以 64 位十六进制数表示的常量
    SPH_C64(0x74EF2CEFEF74C42B), // 以 64 位十六进制数表示的常量
    SPH_C64(0xC33F413F3FC3DAE5), // 以 64 位十六进制数表示的常量
    SPH_C64(0x1C55FF55551CC792), // 以 64 位十六进制数表示的常量
    SPH_C64(0x10A2FBA2A210DB79), // 以 64 位十六进制数表示的常量
    SPH_C64(0x65EA23EAEA65E903), // 以 64 位十六进制数表示的常量
    SPH_C64(0xEC65AF6565EC6A0F), // 以 64 位十六进制数表示的常量
    SPH_C64(0x68BAD3BABA6803B9), // 以 64 位十六进制数表示的常量
    SPH_C64(0x932F712F2F934A65), // 以 64 位十六进制数表示的常量
    SPH_C64(0xE7C05DC0C0E78E4E), // 以 64 位十六进制数表示的常量
    SPH_C64(0x81DE7FDEDE8160BE), // 以 64 位十六进制数表示的常量
    SPH_C64(0x6C1C241C1C6CFCE0), // 以 64 位十六进制数表示的常量
    SPH_C64(0x2EFD1AFDFD2E46BB), // 以 64 位十六进制数表示的常量
    SPH_C64(0x644DD74D4D641F52), // 以 64 位十六进制数表示的常量
    SPH_C64(0xE092AB9292E076E4), // 以 64 位十六进制数表示的常量
    SPH_C64(0xBC759F7575BCFA8F), // 以 64 位十六进制数表示的常量
    SPH_C64(0x1E060A06061E3630), // 以 64 位十六进制数表示的常量
    SPH_C64(0x988A838A8A98AE24), // 以 64 位十六进制数表示的常量
    SPH_C64(0x40B2CBB2B2404BF9), // 以 64 位十六进制数表示的常量
    SPH_C64(0x59E637E6E6598563), // 以 64 位十六进制数表示的常量
    SPH_C64(0x360E120E0E367E70), // 以 64 位十六进制数表示的常量
    SPH_C64(0x631F211F1F63E7F8), // 以 64 位十六进制数表示的常量
    SPH_C64(0xF762A66262F75537), // 以 64 位十六进制数表示的常量
    SPH_C64(0xA3D461D4D4A33AEE), // 以 64 位十六进制数表示的常量
    SPH_C64(0x32A8E5A8A8328129), // 以 64 位十六进制数表示的常量
    SPH_C64(0xF496A79696F452C4), // 以 64 位十六进制数表示的常量
    SPH_C64(0x3AF916F9F93A629B), // 以 64 位十六进制数表示的常量
    SPH_C64(0xF6C552C5C5F6A366), // 以 64 位十六进制数表示的常量
    SPH_C64(0xB1256F2525B11035), // 以 64 位十六进制数表示的常量
    SPH_C64(0x2059EB595920ABF2), // 以 64 位十六进制数表示的常量
    SPH_C64(0xAE84918484AED054), // 以 64 位十六进制数表示的常量
    SPH_C64(0xA772967272A7C5B7), // 以 64 位十六进制数表示的常量
    SPH_C64(0xDD394B3939DDECD5), // 以 64 位十六进制数表示的常量
    SPH_C64(0x614CD44C4C61165A), // 以 64 位十六进制数表示的常量
    SPH_C64(0x3B5EE25E5E3B94CA), // 以 64 位十六进制数表示的常量
    SPH_C64(0x8578887878859FE7), // 以 64 位十六进制数表示的常量
    SPH_C64(0xD838483838D8E5DD), // 以 64 位十六进制数表示的常量
    SPH_C64(0x868C898C8C869814), // 以 64 位十六进制数表示的常量
    SPH_C64(0xB2D16ED1D1B217C6), // 以 64 位十六进制数表示的常量
    SPH_C64(0x0BA5F2A5A50BE441), // 以 64 位十六进制数表示的常量
    SPH_C64(0x4DE23BE2E24DA143), // 以 64 位十六进制数表示的常量
    SPH_C64(0xF861A36161F84E2F), // 以 64 位十六进制数表示的常量
    SPH_C64(0x45B3C8B3B34542F1), // 以 64 位十六进制数表示的常量
    SPH_C64(0xA521632121A53415), // 以 64 位十六进制数表示的常量
    SPH_C64(0xD69CB99C9CD60894), // 以 64 位十六进制数表示的常量
    SPH_C64(0x661E221E1E66EEF0), // 以 64 位十六进制数表示的常量
    SPH_C64(0x5243C54343526122), // 以 64 位十六进制数表示的常量
    SPH_C64(0xFCC754C7C7FCB176), // 以 64 位十六进制数表示的常量
    # 定义一系列64位的常量，使用SPH_C64函数进行转换
...
# 定义一个名为 old1_T4 的静态常量数组，包含 256 个 64 位的常量
static const sph_u64 old1_T4[256] = {
    # 以下是 256 个 64 位常量的十六进制表示
    ...
    SPH_C64(0x8D8A8D8D83911C83),  // 以 64 位常量表示十六进制数
    SPH_C64(0x3D473D3DC9C8F5C9),  // 以 64 位常量表示十六进制数
    SPH_C64(0x97A49797F15BCCF1),  // 以 64 位常量表示十六进制数
    SPH_C64(0x0000000000000000),  // 以 64 位常量表示十六进制数
    SPH_C64(0xCF4CCFCFD4F936D4),  // 以 64 位常量表示十六进制数
    SPH_C64(0x2B7D2B2B876E4587),  // 以 64 位常量表示十六进制数
    SPH_C64(0x769A7676B3E197B3),  // 以 64 位常量表示十六进制数
    SPH_C64(0x829B8282B0E664B0),  // 以 64 位常量表示十六进制数
    SPH_C64(0xD667D6D6A928FEA9),  // 以 64 位常量表示十六进制数
    SPH_C64(0x1B2D1B1B77C3D877),  // 以 64 位常量表示十六进制数
    SPH_C64(0xB5C2B5B55B74C15B),  // 以 64 位常量表示十六进制数
    SPH_C64(0xAFECAFAF29BE1129),  // 以 64 位常量表示十六进制数
    SPH_C64(0x6ABE6A6ADF1D77DF),  // 以 64 位常量表示十六进制数
    SPH_C64(0x50F050500DEABA0D),  // 以 64 位常量表示十六进制数
    SPH_C64(0x45CF45454C57124C),  // 以 64 位常量表示十六进制数
    SPH_C64(0xF308F3F31838CB18),  // 以 64 位常量表示十六进制数
    SPH_C64(0x30503030F0AD9DF0),  // 以 64 位常量表示十六进制数
    SPH_C64(0xEF2CEFEF74C42B74),  // 以 64 位常量表示十六进制数
    SPH_C64(0x3F413F3FC3DAE5C3),  // 以 64 位常量表示十六进制数
    SPH_C64(0x55FF55551CC7921C),  // 以 64 位常量表示十六进制数
    SPH_C64(0xA2FBA2A210DB7910),  // 以 64 位常量表示十六进制数
    SPH_C64(0xEA23EAEA65E90365),  // 以 64 位常量表示十六进制数
    SPH_C64(0x65AF6565EC6A0FEC),  // 以 64 位常量表示十六进制数
    SPH_C64(0xBAD3BABA6803B968),  // 以 64 位常量表示十六进制数
    SPH_C64(0x2F712F2F934A6593),  // 以 64 位常量表示十六进制数
    SPH_C64(0xC05DC0C0E78E4EE7),  // 以 64 位常量表示十六进制数
    SPH_C64(0xDE7FDEDE8160BE81),  // 以 64 位常量表示十六进制数
    SPH_C64(0x1C241C1C6CFCE06C),  // 以 64 位常量表示十六进制数
    SPH_C64(0xFD1AFDFD2E46BB2E),  // 以 64 位常量表示十六进制数
    SPH_C64(0x4DD74D4D641F5264),  // 以 64 位常量表示十六进制数
    SPH_C64(0x92AB9292E076E4E0),  // 以 64 位常量表示十六进制数
    SPH_C64(0x759F7575BCFA8FBC),  // 以 64 位常量表示十六进制数
    SPH_C64(0x060A06061E36301E),  // 以 64 位常量表示十六进制数
    SPH_C64(0x8A838A8A98AE2498),  // 以 64 位常量表示十六进制数
    SPH_C64(0xB2CBB2B2404BF940),  // 以 64 位常量表示十六进制数
    SPH_C64(0xE637E6E659856359),  // 以 64 位常量表示十六进制数
    SPH_C64(0x0E120E0E367E7036),  // 以 64 位常量表示十六进制数
    SPH_C64(0x1F211F1F63E7F863),  // 以 64 位常量表示十六进制数
    SPH_C64(0x62A66262F75537F7),  // 以 64 位常量表示十六进制数
    SPH_C64(0xD461D4D4A33AEEA3),  // 以 64 位常量表示十六进制数
    SPH_C64(0xA8E5A8A832812932),  // 以 64 位常量表示十六进制数
    SPH_C64(0x96A79696F452C4F4),  // 以 64 位常量表示十六进制数
    SPH_C64(0xF916F9F93A629B3A),  // 以 64 位常量表示十六进制数
    SPH_C64(0xC552C5C5F6A366F6),  // 以 64 位常量表示十六进制数
    SPH_C64(0x256F2525B11035B1),  // 以 64 位常量表示十六进制数
    SPH_C64(0x59EB595920ABF220),  // 以 64 位常量表示十六进制数
    SPH_C64(0x84918484AED054AE),  // 以 64 位常量表示十六进制数
    SPH_C64(0x72967272A7C5B7A7),  // 以 64 位常量表示十六进制数
    SPH_C64(0x394B3939DDECD5DD),  // 以 64 位常量表示十六进制数
    SPH_C64(0x4CD44C4C61165A61),  // 以 64 位常量表示十六进制数
    SPH_C64(0x5EE25E5E3B94CA3B),  // 以 64 位常量表示十六进制数
    SPH_C64(0x78887878859FE785),  // 以 64 位常量表示十六进制数
    SPH_C64(0x38483838D8E5DDD8),  // 以 64 位常量表示十六进制数
    SPH_C64(0x8C898C8C86981486),  // 以 64 位常量表示十六进制数
    SPH_C64(0xD16ED1D1B217C6B2),  // 以 64 位常量表示十六进制数
    SPH_C64(0xA5F2A5A50BE4410B),  // 以 64 位常量表示十六进制数
    SPH_C64(0xE23BE2E24DA1434D),  // 以 64 位常量表示十六进制数
    SPH_C64(0x61A36161F84E2FF8),  // 以 64 位常量表示十六进制数
    SPH_C64(0xB3C8B3B34542F145),  // 以 64 位常量表示十六进制数
    SPH_C64(0x21632121A53415A5),  // 以 64 位常量表示十六进制数
    SPH_C64(0x9CB99C9CD60894D6),  // 以 64 位常量表示十六进制数
    SPH_C64(0x1E221E1E66EEF066),  // 以 64 位常量表示十六进制数
    SPH_C64(0x43C5434352612252),  // 以 64 位常量表示十六进制数
    SPH_C64(0xC754C7C7FCB176FC),  // 以 64 位常量表示十六进制数
    # 定义一系列64位常量，并使用SPH_C64函数进行转换
...
static const sph_u64 old1_T5[256] = {
    ...
    # 使用SPH_C64函数对0x15F8F83F6B933FF8进行处理
    SPH_C64(0x15F8F83F6B933FF8), 
    # 使用SPH_C64函数对0x978686A4C244A486进行处理
    SPH_C64(0x978686A4C244A486)
# 定义一个静态的64位无符号整数数组，用于存储一些常量值
static const sph_u64 old1_T6[256] = {
    ...
    SPH_C64(0x8D8D83911C838D8A),  // 将十六进制数转换为64位整数
    SPH_C64(0x3D3DC9C8F5C93D47),
    SPH_C64(0x9797F15BCCF197A4),
    SPH_C64(0x0000000000000000),
    SPH_C64(0xCFCFD4F936D4CF4C),
    SPH_C64(0x2B2B876E45872B7D),
    SPH_C64(0x7676B3E197B3769A),
    SPH_C64(0x8282B0E664B0829B),
    SPH_C64(0xD6D6A928FEA9D667),
    SPH_C64(0x1B1B77C3D8771B2D),
    SPH_C64(0xB5B55B74C15BB5C2),
    SPH_C64(0xAFAF29BE1129AFEC),
    SPH_C64(0x6A6ADF1D77DF6ABE),
    SPH_C64(0x50500DEABA0D50F0),
    SPH_C64(0x45454C57124C45CF),
    SPH_C64(0xF3F31838CB18F308),
    SPH_C64(0x3030F0AD9DF03050),
    SPH_C64(0xEFEF74C42B74EF2C),
    SPH_C64(0x3F3FC3DAE5C33F41),
    SPH_C64(0x55551CC7921C55FF),
    SPH_C64(0xA2A210DB7910A2FB),
    SPH_C64(0xEAEA65E90365EA23),
    SPH_C64(0x6565EC6A0FEC65AF),
    SPH_C64(0xBABA6803B968BAD3),
    SPH_C64(0x2F2F934A65932F71),
    SPH_C64(0xC0C0E78E4EE7C05D),
    SPH_C64(0xDEDE8160BE81DE7F),
    SPH_C64(0x1C1C6CFCE06C1C24),
    SPH_C64(0xFDFD2E46BB2EFD1A),
    SPH_C64(0x4D4D641F52644DD7),
    SPH_C64(0x9292E076E4E092AB),
    SPH_C64(0x7575BCFA8FBC759F),
    SPH_C64(0x06061E36301E060A),
    SPH_C64(0x8A8A98AE24988A83),
    SPH_C64(0xB2B2404BF940B2CB),
    SPH_C64(0xE6E659856359E637),
    SPH_C64(0x0E0E367E70360E12),
    SPH_C64(0x1F1F63E7F8631F21),
    SPH_C64(0x6262F75537F762A6),
    SPH_C64(0xD4D4A33AEEA3D461),
    SPH_C64(0xA8A832812932A8E5),
    SPH_C64(0x9696F452C4F496A7),
    SPH_C64(0xF9F93A629B3AF916),
    SPH_C64(0xC5C5F6A366F6C552),
    SPH_C64(0x2525B11035B1256F),
    SPH_C64(0x595920ABF22059EB),
    SPH_C64(0x8484AED054AE8491),
    SPH_C64(0x7272A7C5B7A77296),
    SPH_C64(0x3939DDECD5DD394B),
    SPH_C64(0x4C4C61165A614CD4),
    SPH_C64(0x5E5E3B94CA3B5EE2),
    SPH_C64(0x7878859FE7857888),
    SPH_C64(0x3838D8E5DDD83848),
    SPH_C64(0x8C8C869814868C89),
    SPH_C64(0xD1D1B217C6B2D16E),
    SPH_C64(0xA5A50BE4410BA5F2),
    SPH_C64(0xE2E24DA1434DE23B),
    SPH_C64(0x6161F84E2FF861A3),
    SPH_C64(0xB3B34542F145B3C8),
    SPH_C64(0x2121A53415A52163),
    SPH_C64(0x9C9CD60894D69CB9),
    SPH_C64(0x1E1E66EEF0661E22),
    SPH_C64(0x43435261225243C5),
    SPH_C64(0xC7C7FCB176FCC754),
...
static const sph_u64 old1_T7[256] = {
    ...
    SPH_C64(0x8D83911C838D8A8D), // 64位常量，用于SHA-512算法
    SPH_C64(0x3DC9C8F5C93D473D), // 64位常量，用于SHA-512算法
    SPH_C64(0x97F15BCCF197A497), // 64位常量，用于SHA-512算法
    SPH_C64(0x0000000000000000), // 64位常量，用于SHA-512算法
    SPH_C64(0xCFD4F936D4CF4CCF), // 64位常量，用于SHA-512算法
    SPH_C64(0x2B876E45872B7D2B), // 64位常量，用于SHA-512算法
    SPH_C64(0x76B3E197B3769A76), // 64位常量，用于SHA-512算法
    SPH_C64(0x82B0E664B0829B82), // 64位常量，用于SHA-512算法
    SPH_C64(0xD6A928FEA9D667D6), // 64位常量，用于SHA-512算法
    SPH_C64(0x1B77C3D8771B2D1B), // 64位常量，用于SHA-512算法
    SPH_C64(0xB55B74C15BB5C2B5), // 64位常量，用于SHA-512算法
    SPH_C64(0xAF29BE1129AFECAF), // 64位常量，用于SHA-512算法
    SPH_C64(0x6ADF1D77DF6ABE6A), // 64位常量，用于SHA-512算法
    SPH_C64(0x500DEABA0D50F050), // 64位常量，用于SHA-512算法
    SPH_C64(0x454C57124C45CF45), // 64位常量，用于SHA-512算法
    SPH_C64(0xF31838CB18F308F3), // 64位常量，用于SHA-512算法
    SPH_C64(0x30F0AD9DF0305030), // 64位常量，用于SHA-512算法
    SPH_C64(0xEF74C42B74EF2CEF), // 64位常量，用于SHA-512算法
    SPH_C64(0x3FC3DAE5C33F413F), // 64位常量，用于SHA-512算法
    SPH_C64(0x551CC7921C55FF55), // 64位常量，用于SHA-512算法
    SPH_C64(0xA210DB7910A2FBA2), // 64位常量，用于SHA-512算法
    SPH_C64(0xEA65E90365EA23EA), // 64位常量，用于SHA-512算法
    SPH_C64(0x65EC6A0FEC65AF65), // 64位常量，用于SHA-512算法
    SPH_C64(0xBA6803B968BAD3BA), // 64位常量，用于SHA-512算法
    SPH_C64(0x2F934A65932F712F), // 64位常量，用于SHA-512算法
    SPH_C64(0xC0E78E4EE7C05DC0), // 64位常量，用于SHA-512算法
    SPH_C64(0xDE8160BE81DE7FDE), // 64位常量，用于SHA-512算法
    SPH_C64(0x1C6CFCE06C1C241C), // 64位常量，用于SHA-512算法
    SPH_C64(0xFD2E46BB2EFD1AFD), // 64位常量，用于SHA-512算法
    SPH_C64(0x4D641F52644DD74D), // 64位常量，用于SHA-512算法
    SPH_C64(0x92E076E4E092AB92), // 64位常量，用于SHA-512算法
    SPH_C64(0x75BCFA8FBC759F75), // 64位常量，用于SHA-512算法
    SPH_C64(0x061E36301E060A06), // 64位常量，用于SHA-512算法
    SPH_C64(0x8A98AE24988A838A), // 64位常量，用于SHA-512算法
    SPH_C64(0xB2404BF940B2CBB2), // 64位常量，用于SHA-512算法
    SPH_C64(0xE659856359E637E6), // 64位常量，用于SHA-512算法
    SPH_C64(0x0E367E70360E120E), // 64位常量，用于SHA-512算法
    SPH_C64(0x1F63E7F8631F211F), // 64位常量，用于SHA-512算法
    SPH_C64(0x62F75537F762A662), // 64位常量，用于SHA-512算法
    SPH_C64(0xD4A33AEEA3D461D4), // 64位常量，用于SHA-512算法
    SPH_C64(0xA832812932A8E5A8), // 64位常量，用于SHA-512算法
    SPH_C64(0x96F452C4F496A796), // 64位常量，用于SHA-512算法
    SPH_C64(0xF93A629B3AF916F9), // 64位常量，用于SHA-512算法
    SPH_C64(0xC5F6A366F6C552C5), // 64位常量，用于SHA-512算法
    SPH_C64(0x25B11035B1256F25), // 64位常量，用于SHA-512算法
    SPH_C64(0x5920ABF22059EB59), // 64位常量，用于SHA-512算法
    SPH_C64(0x84AED054AE849184), // 64位常量，用于SHA-512算法
    SPH_C64(0x72A7C5B7A7729672), // 64位常量，用于SHA-512算法
    SPH_C64(0x39DDECD5DD394B39), // 64位常量，用于SHA-512算法
    SPH_C64(0x4C61165A614CD44C), // 64位常量，用于SHA-512算法
    SPH_C64(0x5E3B94CA3B5EE25E), // 64位常量，用于SHA-512算法
    SPH_C64(0x78859FE785788878), // 64位常量，用于SHA-512算法
    SPH_C64(0x38D8E5DDD8384838), // 64位常量，用于SHA-512算法
    SPH_C64(0x8C869814868C898C), // 64位常量，用于SHA-512算法
    SPH_C64(0xD1B217C6B2D16ED1), // 64位常量，用于SHA-512算法
    SPH_C64(0xA50BE4410BA5F2A5), // 64位常量，用于SHA-512算法
    SPH_C64(0xE24DA1434DE23BE2), // 64位常量，用于SHA-512算法
    SPH_C64(0x61F84E2FF861A361), // 64位常量，用于SHA-512算法
    SPH_C64(0xB34542F145B3C8B3), // 64位常量，用于SHA-512算法
    SPH_C64(0x21A53415A5216321), // 64位常量，用于SHA-512算法
    SPH_C64(0x9CD60894D69CB99C), // 64位常量，用于SHA-512算法
    SPH_C64(0x1E66EEF0661E221E), // 64位常量，用于SHA-512算法
    SPH_C64(0x435261225243C543), // 64位常量，用于SHA-512算法
    SPH_C64(0xC7FCB176FCC754C7), // 64位常量，用于SHA-512算法
    SPH_C64(0xFC2B4FB32BFC19FC),  // 用于哈希计算的常量
    SPH_C64(0x0414242014040C04),  // 用于哈希计算的常量
    SPH_C64(0x5108E3B20851F351),  // 用于哈希计算的常量
    SPH_C64(0x99C725BCC799B699),  // 用于哈希计算的常量
    SPH_C64(0x6DC4224FC46DB76D),  // 用于哈希计算的常量
    SPH_C64(0x0D396568390D170D),  // 用于哈希计算的常量
    SPH_C64(0xFA35798335FA13FA),  // 用于哈希计算的常量
    SPH_C64(0xDF8469B684DF7CDF),  // 用于哈希计算的常量
    SPH_C64(0x7E9BA9D79B7E827E),  // 用于哈希计算的常量
    SPH_C64(0x24B4193DB4246C24),  // 用于哈希计算的常量
    SPH_C64(0x3BD7FEC5D73B4D3B),  // 用于哈希计算的常量
    SPH_C64(0xAB3D9A313DABE0AB),  // 用于哈希计算的常量
    SPH_C64(0xCED1F03ED1CE4FCE),  // 用于哈希计算的常量
    SPH_C64(0x1155998855113311),  // 用于哈希计算的常量
    SPH_C64(0x8F89830C898F8C8F),  // 用于哈希计算的常量
    SPH_C64(0x4E6B044A6B4ED24E),  // 用于哈希计算的常量
    SPH_C64(0xB75166D151B7C4B7),  // 用于哈希计算的常量
    SPH_C64(0xEB60E00B60EB20EB),  // 用于哈希计算的常量
    SPH_C64(0x3CCCC1FDCC3C443C),  // 用于哈希计算的常量
    SPH_C64(0x81BFFD7CBF819E81),  // 用于哈希计算的常量
    SPH_C64(0x94FE40D4FE94A194),  // 用于哈希计算的常量
    SPH_C64(0xF70C1CEB0CF704F7),  // 用于哈希计算的常量
    SPH_C64(0xB96718A167B9D6B9),  // 用于哈希计算的常量
    SPH_C64(0x135F8B985F133513),  // 用于哈希计算的常量
    SPH_C64(0x2C9C517D9C2C742C),  // 用于哈希计算的常量
    SPH_C64(0xD3B805D6B8D368D3),  // 用于哈希计算的常量
    SPH_C64(0xE75C8C6B5CE734E7),  // 用于哈希计算的常量
    SPH_C64(0x6ECB3957CB6EB26E),  // 用于哈希计算的常量
    SPH_C64(0xC4F3AA6EF3C451C4),  // 用于哈希计算的常量
    SPH_C64(0x030F1B180F030503),  // 用于哈希计算的常量
    SPH_C64(0x5613DC8A1356FA56),  // 用于哈希计算的常量
    SPH_C64(0x44495E1A4944CC44),  // 用于哈希计算的常量
    SPH_C64(0x7F9EA0DF9E7F817F),  // 用于哈希计算的常量
    SPH_C64(0xA937882137A9E6A9),  // 用于哈希计算的常量
    SPH_C64(0x2A82674D822A7E2A),  // 用于哈希计算的常量
    SPH_C64(0xBB6D0AB16DBBD0BB),  // 用于哈希计算的常量
    SPH_C64(0xC1E28746E2C15EC1),  // 用于哈希计算的常量
    SPH_C64(0x5302F1A20253F553),  // 用于哈希计算的常量
    SPH_C64(0xDC8B72AE8BDC79DC),  // 用于哈希计算的常量
    SPH_C64(0x0B275358270B1D0B),  // 用于哈希计算的常量
    SPH_C64(0x9DD3019CD39DBA9D),  // 用于哈希计算的常量
    SPH_C64(0x6CC12B47C16CB46C),  // 用于哈希计算的常量
    SPH_C64(0x31F5A495F5315331),  // 用于哈希计算的常量
    SPH_C64(0x74B9F387B9749C74),  // 用于哈希计算的常量
    SPH_C64(0xF60915E309F607F6),  // 用于哈希计算的常量
    SPH_C64(0x46434C0A4346CA46),  // 用于哈希计算的常量
    SPH_C64(0xAC26A50926ACE9AC),  // 用于哈希计算的常量
    SPH_C64(0x8997B53C97898689),  // 用于哈希计算的常量
    SPH_C64(0x1444B4A044143C14),  // 用于哈希计算的常量
    SPH_C64(0xE142BA5B42E13EE1),  // 用于哈希计算的常量
    SPH_C64(0x164EA6B04E163A16),  // 用于哈希计算的常量
    SPH_C64(0x3AD2F7CDD23A4E3A),  // 用于哈希计算的常量
    SPH_C64(0x69D0066FD069BB69),  // 用于哈希计算的常量
    SPH_C64(0x092D41482D091B09),  // 用于哈希计算的常量
    SPH_C64(0x70ADD7A7AD709070),  // 用于哈希计算的常量
    SPH_C64(0xB6546FD954B6C7B6),  // 用于哈希计算的常量
    SPH_C64(0xD0B71ECEB7D06DD0),  // 用于哈希计算的常量
    SPH_C64(0xED7ED63B7EED2AED),  // 用于哈希计算的常量
    SPH_C64(0xCCDBE22EDBCC49CC),  // 用于哈希计算的常量
    SPH_C64(0x4257682A5742C642),  // 用于哈希计算的常量
    SPH_C64(0x98C22CB4C298B598),  // 用于哈希计算的常量
    SPH_C64(0xA40EED490EA4F1A4),  // 用于哈希计算的常量
    SPH_C64(0x2888755D88287828),  // 用于哈希计算的常量
    SPH_C64(0x5C3186DA315CE45C),  // 用于哈希计算的常量
    # 这是一个函数调用，传入两个参数
    # 参数1: 0xF83F6B933FF815F8，使用SPH_C64函数对十六进制数进行处理
    # 参数2: 0x86A4C244A4869786，使用SPH_C64函数对十六进制数进行处理
};

#endif

static const sph_u64 old1_RC[10] = {
    SPH_C64(0x4F01B887E8C62318),  // 定义一个包含10个64位整数的常量数组
    SPH_C64(0x52916F79F5D2A636),
    SPH_C64(0x357B0CA38E9BBC60),
    SPH_C64(0x57FE4B2EC2D7E01D),
    SPH_C64(0xDA4AF09FE5377715),
...
#define DECL8(z)   sph_u64 z ## 0, z ## 1, z ## 2, z ## 3, \  // 定义一个宏，用于声明8个sph_u64类型的变量
                   z ## 4, z ## 5, z ## 6, z ## 7

#if SPH_LITTLE_FAST
#define READ_DATA_W(x)   do { \  // 定义一个宏，用于读取输入数据的宏
        n ## x = sph_dec64le_aligned( \  // 将输入数据按64位小端格式解码并赋值给n##x
            (const unsigned char *)src + 8 * (x)); \
    } while (0)
#define UPDATE_STATE_W(x)   do { \  // 定义一个宏，用于更新状态的宏
        state[x] ^= n ## x ^ sph_dec64le_aligned( \  // 更新状态
            (const unsigned char *)src + 8 * (x)); \
    } while (0)
#define LVARS   DECL8(n); DECL8(h);  // 定义一个宏，用于声明n和h
#else
#define READ_DATA_W(x)   do { \  // 定义一个宏，用于读取输入数据的宏
        sn ## x = n ## x = sph_dec64le_aligned( \  // 将输入数据按64位小端格式解码并赋值给sn##x和n##x
            (const unsigned char *)src + 8 * (x)); \
    } while (0)
#define UPDATE_STATE_W(x)   do { \  // 定义一个宏，用于更新状态的宏
        state[x] ^= n ## x ^ sn ## x; \  // 更新状态
    } while (0)
#define LVARS   DECL8(n); DECL8(sn); DECL8(h);  // 定义一个宏，用于声明n、sn和h
#endif

#define READ_STATE_W(x)   do { h ## x = state[x]; } while (0)  // 定义一个宏，用于读取状态的宏

#define MUL8(FUN)   do { \  // 定义一个宏，用于执行8次指定的操作
        FUN(0); \
        FUN(1); \
        FUN(2); \
        FUN(3); \
 ...
 */
#define ROUND0_W(x)   do { \  // 定义一个宏，用于执行第一轮操作
        n ## x ^= h ## x; \  // 对输入数据和第一轮密钥进行异或操作
    } while (0)

#define READ_DATA      MUL8(READ_DATA_W)  // 定义一个宏，用于执行8次读取输入数据的操作
#define READ_STATE     MUL8(READ_STATE_W)  // 定义一个宏，用于执行8次读取状态的操作
#define ROUND0         MUL8(ROUND0_W)  // 定义一个宏，用于执行8次第一轮操作
#define UPDATE_STATE   MUL8(UPDATE_STATE_W)  // 定义一个宏，用于执行8次更新状态的操作

#define BYTE(x, n)     ((unsigned)((x) >> (8 * (n))) & 0xFF)  // 定义一个宏，用于获取64位整数x的第n个字节的值
...
{
    return SPH_ROTL64(val, 8 * num);  // 定义一个函数，用于对64位整数进行循环左移操作
}
#define ROUND_ELT(table, in, i0, i1, i2, i3, i4, i5, i6, i7) \  
    // 定义一个宏，用于计算轮函数中的一个元素，根据不同的条件编译选择不同的实现
    (table ## 0[BYTE(in ## i0, 0)] \  
    // 使用宏 BYTE 计算输入的索引值，然后根据索引值从 table 中取出对应的值，并进行异或操作
    ^ table_skew(table ## 0[BYTE(in ## i1, 1)], 1) \  
    // 使用宏 BYTE 计算输入的索引值，然后根据索引值从 table 中取出对应的值，再进行 table_skew 函数处理，最后进行异或操作
...
#define ROUND(table, in, out, c0, c1, c2, c3, c4, c5, c6, c7)   do { \
    // 定义一个宏，用于计算轮函数中的一个完整轮，根据不同的条件编译选择不同的实现
        out ## 0 = ROUND_ELT(table, in, 0, 7, 6, 5, 4, 3, 2, 1) ^ c0; \
        // 调用 ROUND_ELT 宏计算每个元素的值，并进行异或操作
        out ## 1 = ROUND_ELT(table, in, 1, 0, 7, 6, 5, 4, 3, 2) ^ c1; \
        // 依此类推，计算每个元素的值，并进行异或操作
...
将src的值传递给dst，其中dst和src是以0到7结尾的变量。
...
初始化sph_whirlpool_context结构体，将其state成员的值设置为0，并根据SPH_64的值设置count或count_high和count_low的值。
...
定义一个宏，用于生成名为name_round的函数，该函数接受src和state作为参数。在函数内部，它使用一些宏和变量来执行一系列操作。
...
使用ROUND_FUN宏生成whirlpool_round、whirlpool0_round和whirlpool1_round函数。
...
定义一些宏，然后使用这些宏包含md_helper.c文件的内容。
...
定义一个宏，用于生成名为sph_name_close的函数，该函数接受cc和dst作为参数。
    # 定义一个指向 sph_ ## name ## _context 结构体的指针变量 sc
    sph_ ## name ## _context *sc; \
    # 定义一个整型变量 i
    int i; \
    # 调用 name ## _close 函数，关闭 cc 指向的资源，并将结果保存到 dst 中
    name ## _close(cc, dst, 0); \
    # 将 cc 的值赋给 sc
    sc = cc; \
    # 循环 8 次
    for (i = 0; i < 8; i ++) \
        # 将 sc->state[i] 的值以小端序写入 dst 的相应位置
...
这段代码是一个注释块，注释了一段被注释掉的代码。这段代码可能是一个宏定义或函数定义的一部分。
...
这三行代码调用了名为`MAKE_CLOSE`的宏，并传入了不同的参数`whirlpool`、`whirlpool0`和`whirlpool1`。这些宏可能用于关闭与这些参数相关的资源或执行其他操作。
...
这段代码是一个条件编译块，用于在C++环境下关闭对应的C语言的命名空间。如果当前环境是C++，则会关闭对应的命名空间。
```