...
{
    setArg(3, sizeof(uint32_t), &iteration);  // 设置内核参数，用于指定迭代次数

    const size_t gthreads        = threads * 8;  // 计算全局工作组大小
    static const size_t lthreads = 32;  // 设置本地工作组大小

    enqueueNDRange(queue, 1, nullptr, &gthreads, &lthreads);  // 将内核加入到命令队列中并执行
}
...
{
    setArg(0, sizeof(cl_mem), &entropy_data);  // 设置内核参数，用于指定熵数据
    setArg(1, sizeof(cl_mem), &vm_states);  // 设置内核参数，用于指定虚拟机状态
    setArg(2, sizeof(cl_mem), &rounding);  // 设置内核参数，用于指定舍入值
}
```